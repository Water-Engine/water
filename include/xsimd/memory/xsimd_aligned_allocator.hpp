 ...
 */
template <class T, size_t Align = std::conditional<std::is_same<unsupported, default_arch>::value,
                                                   common, default_arch>::type::alignment()>
class aligned_allocator {
  public:
    using value_type = T;
    using pointer = T*;
 ...
 * Default constructor.
 */
template <class T, size_t A> XSIMD_INLINE aligned_allocator<T, A>::aligned_allocator() noexcept {}
 ...
 * Destructor.
 */
template <class T, size_t A> XSIMD_INLINE aligned_allocator<T, A>::~aligned_allocator() {}
 ...
 */
template <class T, size_t A>
XSIMD_INLINE auto aligned_allocator<T, A>::address(reference r) noexcept -> pointer {
    return &r;
}
 ...
 */
template <class T, size_t A>
XSIMD_INLINE auto aligned_allocator<T, A>::address(const_reference r) const noexcept
    -> const_pointer {
    return &r;
}
 ...
 */
template <class T, size_t A>
XSIMD_INLINE auto aligned_allocator<T, A>::allocate(size_type n, const void*) -> pointer {
 ...
 */
template <class T, size_t A>
XSIMD_INLINE auto aligned_allocator<T, A>::max_size() const noexcept -> size_type {
 ...
 */
template <class T, size_t A>
XSIMD_INLINE auto aligned_allocator<T, A>::size_max() const noexcept -> size_type {
    return size_type(-1) / sizeof(T);
}
...
XSIMD_INLINE void aligned_free(void* ptr) { detail::xaligned_free(ptr); }
...
XSIMD_INLINE size_t get_alignment_offset(const T* p, size_t size, size_t block_size) {
    ...
    if (block_size == 1) {
        ...
        return std::min<size_t>((block_size - ((size_t(p) / sizeof(T)) & block_mask)) & block_mask,
                                size);
    }
}
...
using default_allocator =
    typename std::conditional<A::requires_alignment(), aligned_allocator<T, A::alignment()>,
