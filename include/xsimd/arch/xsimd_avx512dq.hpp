...
#define XSIMD_AVX512_HADDP_STEP2(I, a, b, c, d)                                                    \
    ...
    {                                                                                              \
        ...
        auto resx3 = _mm512_add_ps(tmp5, tmp6);                                                    \
                                                                                                   \
        halfx##I =                                                                                 \
            _mm256_hadd_ps(_mm512_extractf32x8_ps(resx3, 0), _mm512_extractf32x8_ps(resx3, 1));    \
    }
...
// mul
template <class A>
XSIMD_INLINE batch<uint64_t, A> mul(batch<uint64_t, A> const& self, batch<uint64_t, A> const& other,
                                    requires_arch<avx512dq>) noexcept {
    return _mm512_mullo_epi64(self, other);
}

template <class A>
XSIMD_INLINE batch<int64_t, A> mul(batch<int64_t, A> const& self, batch<int64_t, A> const& other,
                                   requires_arch<avx512dq>) noexcept {
    return _mm512_mullo_epi64(self, other);
}
...
XSIMD_INLINE batch<float, A> swizzle(batch<float, A> const& self,
    ...
    XSIMD_IF_CONSTEXPR(dup_lo || dup_hi) {
        ...
        constexpr typename std::conditional<
            dup_lo,
            batch_constant<uint32_t, avx2, V0 % 8, V1 % 8, V2 % 8, V3 % 8, V4 % 8, V5 % 8, V6 % 8,
                           V7 % 8>,
            batch_constant<uint32_t, avx2, V8 % 8, V9 % 8, V10 % 8, V11 % 8, V12 % 8, V13 % 8,
                           V14 % 8, V15 % 8>>::type half_mask{};
