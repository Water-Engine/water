...
XSIMD_INLINE batch<T, A> abs(batch<T, A> const& self, requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_abs(self); }
...
XSIMD_INLINE batch<T, A> add(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_add(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> avgr(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_u8x16_avgr(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> bitwise_lshift(batch<T, A> const& self, int32_t other,
                                        requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_shl(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> bitwise_rshift(batch<T, A> const& self, int32_t other,
    ...
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_shr(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_u8x16_shr(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
// broadcast
template <class A> batch<float, A> XSIMD_INLINE broadcast(float val, requires_arch<wasm>) noexcept {
    return wasm_f32x4_splat(val);
}
template <class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch<T, A> broadcast(T val, requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_splat(val); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch_bool<T, A> eq(batch<T, A> const& self, batch<T, A> const& other,
                                 requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_eq(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<double, A> fast_cast(batch<uint64_t, A> const& x, batch<double, A> const&,
    ...
    v128_t xL = wasm_v128_or(wasm_v128_and(mask, x),
                             wasm_v128_andnot(wasm_f64x2_splat(0x0010000000000000), mask)); //  2^52
...
XSIMD_INLINE batch<double, A> fast_cast(batch<int64_t, A> const& x, batch<double, A> const&,
    ...
    xH = wasm_v128_and(
        xH, wasm_i16x8_make(0x0000, 0x0000, 0xFFFF, 0xFFFF, 0x0000, 0x0000, 0xFFFF, 0xFFFF));
...
XSIMD_INLINE batch<int32_t, A> fast_cast(batch<float, A> const& self, batch<int32_t, A> const&,
                                         requires_arch<wasm>) noexcept {
    return wasm_i32x4_make(static_cast<int32_t>(wasm_f32x4_extract_lane(self, 0)),
...
XSIMD_INLINE T first(batch<T, A> const& self, requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_extract_lane(self, 0); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch_bool<T, A> from_mask(batch_bool<T, A> const&, uint64_t mask,
    ...
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
        ...
        return wasm_i32x4_make(lut32[mask & 0xF], lut32[(mask >> 4) & 0xF],
                               lut32[(mask >> 8) & 0xF], lut32[mask >> 12]);
    }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch_bool<T, A> gt(batch<T, A> const& self, batch<T, A> const& other,
                                 requires_arch<wasm>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_gt(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_u8x16_gt(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<float, A> insert(batch<float, A> const& self, float val, index<I> pos,
                                    requires_arch<wasm>) noexcept {
...
XSIMD_INLINE batch<T, A> insert(batch<T, A> const& self, T val, index<I> pos,
                                requires_arch<wasm>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_replace_lane(self, pos, val); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_u8x16_replace_lane(self, pos, val); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
template <class A, size_t I>
XSIMD_INLINE batch<double, A> insert(batch<double, A> const& self, double val, index<I> pos,
                                     requires_arch<wasm>) noexcept {
...
load_complex(batch<float, A> const& hi, batch<float, A> const& lo, requires_arch<wasm>) noexcept {
    return {wasm_i32x4_shuffle(hi, lo, 0, 2, 4, 6), wasm_i32x4_shuffle(hi, lo, 1, 3, 5, 7)};
}
template <class A>
XSIMD_INLINE batch<std::complex<double>, A>
load_complex(batch<double, A> const& hi, batch<double, A> const& lo, requires_arch<wasm>) noexcept {
...
XSIMD_INLINE batch_bool<T, A> lt(batch<T, A> const& self, batch<T, A> const& other,
                                 requires_arch<wasm>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_lt(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_u8x16_lt(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
            ...
            auto xself =
                wasm_v128_xor(self, wasm_i64x2_splat(std::numeric_limits<int64_t>::lowest()));
            auto xother =
                wasm_v128_xor(other, wasm_i64x2_splat(std::numeric_limits<int64_t>::lowest()));
...
XSIMD_INLINE uint64_t mask(batch_bool<T, A> const& self, requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_bitmask(self); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> neg(batch<T, A> const& self, requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_neg(self); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> sadd(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<wasm>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_add_sat(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_u8x16_add_sat(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> select(batch_bool<T, A> const& cond, batch<T, A> const& true_br,
                                batch<T, A> const& false_br, requires_arch<wasm>) noexcept {
    return wasm_v128_or(wasm_v128_and(cond, true_br), wasm_v128_andnot(false_br, cond));
}
...
select(batch_bool<double, A> const& cond, batch<double, A> const& true_br,
       batch<double, A> const& false_br, requires_arch<wasm>) noexcept {
    return wasm_v128_or(wasm_v128_and(cond, true_br), wasm_v128_andnot(false_br, cond));
}
...
XSIMD_INLINE batch<float, A> shuffle(batch<float, A> const& x, batch<float, A> const& y,
                                     batch_constant<ITy, A, I0, I1, I2, I3>,
                                     requires_arch<wasm>) noexcept {
...
XSIMD_INLINE batch<float, A> set(batch<float, A> const&, requires_arch<wasm>,
...
template <class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch<T, A> set(batch<T, A> const&, requires_arch<wasm>, T v0, T v1, T v2,
                             T v3) noexcept {
    return wasm_i32x4_make(v0, v1, v2, v3);
}

template <class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch<T, A> set(batch<T, A> const&, requires_arch<wasm>, T v0, T v1, T v2, T v3, T v4,
...
template <class A, class... Values>
XSIMD_INLINE batch<double, A> set(batch<double, A> const&, requires_arch<wasm>,
                                  Values... values) noexcept {
...
template <class A, class T, class... Values,
          class = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch_bool<T, A> set(batch_bool<T, A> const&, requires_arch<wasm>,
                                  Values... values) noexcept {
    return set(batch<T, A>(), A{}, static_cast<T>(values ? -1LL : 0LL)...).data;
}

template <class A, class... Values>
XSIMD_INLINE batch_bool<float, A> set(batch_bool<float, A> const&, requires_arch<wasm>,
                                      Values... values) noexcept {
...
template <class A, class... Values>
XSIMD_INLINE batch_bool<double, A> set(batch_bool<double, A> const&, requires_arch<wasm>,
                                       Values... values) noexcept {
...
XSIMD_INLINE batch<T, A> ssub(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<wasm>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_sub_sat(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_u8x16_sub_sat(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> sub(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return wasm_i8x16_sub(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<uint16_t, A> swizzle(batch<uint16_t, A> const& self,
                                        batch_constant<uint16_t, A, V0, V1, V2, V3, V4, V5, V6, V7>,
                                        requires_arch<wasm>) noexcept {
...
XSIMD_INLINE batch<uint8_t, A> swizzle(batch<uint8_t, A> const& self,
                                       batch_constant<uint8_t, A, V0, V1, V2, V3, V4, V5, V6, V7,
                                                      V8, V9, V10, V11, V12, V13, V14, V15>,
                                       requires_arch<wasm>) noexcept {
    return wasm_i8x16_shuffle(self, self, V0, V1, V2, V3, V4, V5, V6, V7, V8, V9, V10, V11, V12,
                              V13, V14, V15);
}
...
XSIMD_INLINE batch<T, A> zip_hi(batch<T, A> const& self, batch<T, A> const& other,
                                requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
        return wasm_i8x16_shuffle(self, other, 8, 24, 9, 25, 10, 26, 11, 27, 12, 28, 13, 29, 14, 30,
                                  15, 31);
    }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> zip_lo(batch<T, A> const& self, batch<T, A> const& other,
                                requires_arch<wasm>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
        return wasm_i8x16_shuffle(self, other, 0, 16, 1, 17, 2, 18, 3, 19, 4, 20, 5, 21, 6, 22, 7,
                                  23);
    }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
