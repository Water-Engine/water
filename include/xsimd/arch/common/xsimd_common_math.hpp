...
namespace detail {
template <class A, class T>
XSIMD_INLINE batch<T, A> avg(batch<T, A> const& x, batch<T, A> const& y, std::true_type,
...
template <class A, class T>
XSIMD_INLINE batch<T, A> avg(batch<T, A> const& x, batch<T, A> const& y, std::false_type,
                             std::true_type) noexcept {
    return (x + y) / 2;
}
} // namespace detail
...
template <class A, class T>
XSIMD_INLINE batch<T, A> avg(batch<T, A> const& x, batch<T, A> const& y,
                             requires_arch<common>) noexcept {
    return detail::avg(x, y, typename std::is_integral<T>::type{},
...
template <class A, class T>
XSIMD_INLINE batch<T, A> avgr(batch<T, A> const& x, batch<T, A> const& y,
                              std::false_type) noexcept {
...
// batch_cast
template <class A, class T>
XSIMD_INLINE batch<T, A> batch_cast(batch<T, A> const& self, batch<T, A> const&,
                                    requires_arch<common>) noexcept {
    return self;
}
...
XSIMD_INLINE batch<T_out, A> batch_cast(batch<T_in, A> const& self, batch<T_out, A> const& out,
                                        requires_arch<common>, with_fast_conversion) noexcept {
    return fast_cast(self, out, A{});
}
#if defined(__clang__) || __GNUC__
template <class A, class T_out, class T_in>
XSIMD_INLINE batch<T_out, A> batch_cast(batch<T_in, A> const& self, batch<T_out, A> const&,
                                        requires_arch<common>, with_slow_conversion) noexcept
    __attribute__((no_sanitize("undefined")));
#endif
template <class A, class T_out, class T_in>
XSIMD_INLINE batch<T_out, A> batch_cast(batch<T_in, A> const& self, batch<T_out, A> const&,
                                        requires_arch<common>, with_slow_conversion) noexcept {
    static_assert(!std::is_same<T_in, T_out>::value,
                  "there should be no conversion for this type combination");
...
template <class A>
XSIMD_INLINE batch<float, A> bitofsign(batch<float, A> const& self,
                                       requires_arch<common>) noexcept {
    return self & constants::signmask<batch<float, A>>();
}
template <class A>
XSIMD_INLINE batch<double, A> bitofsign(batch<double, A> const& self,
                                        requires_arch<common>) noexcept {
...
// bitwise_cast
template <class A, class T>
XSIMD_INLINE batch<T, A> bitwise_cast(batch<T, A> const& self, batch<T, A> const&,
                                      requires_arch<common>) noexcept {
    return self;
}
...
XSIMD_INLINE batch<float, A> cbrt(batch<float, A> const& self, requires_arch<common>) noexcept {
    ...
    x = detail::horner<batch_type, 0x3ece0609, 0x3f91eb77, 0xbf745265, 0x3f0bf0fe, 0xbe09e49a>(x);
...
XSIMD_INLINE batch<double, A> cbrt(batch<double, A> const& self, requires_arch<common>) noexcept {
    ...
    i_type e;
    batch_type x = frexp(z, e);
    x = detail::horner<batch_type, 0x3fd9c0c12122a4feull, 0x3ff23d6ee505873aull,
                       0xbfee8a4ca3ba37b8ull, 0x3fe17e1fc7e59d58ull, 0xbfc13c93386fdff6ull>(x);
...
// clip
template <class A, class T>
XSIMD_INLINE batch<T, A> clip(batch<T, A> const& self, batch<T, A> const& lo, batch<T, A> const& hi,
                              requires_arch<common>) noexcept {
...
// copysign
template <class A, class T,
          class _ = typename std::enable_if<std::is_floating_point<T>::value, void>::type>
XSIMD_INLINE batch<T, A> copysign(batch<T, A> const& self, batch<T, A> const& other,
                                  requires_arch<common>) noexcept {
 ...
 * ====================================================
 */
template <class B> struct erf_kernel;

template <class A> struct erf_kernel<batch<float, A>> {
    using batch_type = batch<float, A>;
    // computes erf(a0)/a0
...
template <class A> struct erf_kernel<batch<double, A>> {
    using batch_type = batch<double, A>;
    // computes erf(a0)/a0
...
template <class B> struct estrin {
    B x;

    template <typename... Ts> XSIMD_INLINE B operator()(const Ts&... coefs) noexcept {
        return eval(coefs...);
    }

  private:
    XSIMD_INLINE B eval(const B& c0) noexcept { return c0; }

    XSIMD_INLINE B eval(const B& c0, const B& c1) noexcept { return fma(x, c1, c0); }
    ...
    template <class... Args> XSIMD_INLINE B eval(const std::tuple<Args...>& tuple) noexcept {
    ...
    XSIMD_INLINE B eval(const std::tuple<Args...>& tuple, const B& c0, const B& c1,
                        const Ts&... coefs) noexcept {
...
template <class B, exp_reduction_tag Tag> struct exp_reduction_base;

template <class B> struct exp_reduction_base<B, exp_tag> {
    static constexpr B maxlog() noexcept { return constants::maxlog<B>(); }

    static constexpr B minlog() noexcept { return constants::minlog<B>(); }
};

template <class B> struct exp_reduction_base<B, exp10_tag> {
    static constexpr B maxlog() noexcept { return constants::maxlog10<B>(); }

    static constexpr B minlog() noexcept { return constants::minlog10<B>(); }
};

template <class B> struct exp_reduction_base<B, exp2_tag> {
    static constexpr B maxlog() noexcept { return constants::maxlog2<B>(); }

    static constexpr B minlog() noexcept { return constants::minlog2<B>(); }
};

template <class T, class A, exp_reduction_tag Tag> struct exp_reduction;
...
struct exp_reduction<double, A, exp_tag> : exp_reduction_base<batch<double, A>, exp_tag> {
    ...
    static XSIMD_INLINE batch_type approx(const batch_type& x) noexcept {
        ...
        return fnma(
            t,
            detail::horner<batch_type, 0x3fc555555555553eull, 0xbf66c16c16bebd93ull,
                           0x3f11566aaf25de2cull, 0xbebbbd41c5d26bf1ull, 0x3e66376972bea4d0ull>(t),
            x);
    }

    static XSIMD_INLINE batch_type reduce(const batch_type& a, batch_type& hi, batch_type& lo,
                                          batch_type& x) noexcept {
        batch_type k = nearbyint(constants::invlog_2<batch_type>() * a);
    ...
    static XSIMD_INLINE batch_type finalize(const batch_type& x, const batch_type& c,
                                            const batch_type& hi, const batch_type& lo) noexcept {
...
struct exp_reduction<double, A, exp10_tag> : exp_reduction_base<batch<double, A>, exp10_tag> {
    ...
    static XSIMD_INLINE batch_type approx(const batch_type& x) noexcept {
        batch_type xx = x * x;
        batch_type px = x * detail::horner<batch_type, 0x40a2b4798e134a01ull, 0x40796b7a050349e4ull,
                                           0x40277d9474c55934ull, 0x3fa4fd75f3062dd4ull>(xx);
        batch_type x2 = px / (detail::horner1<batch_type, 0x40a03f37650df6e2ull,
                                              0x4093e05eefd67782ull, 0x405545fdce51ca08ull>(xx) -
                              px);
        return ++(x2 + x2);
    }

    static XSIMD_INLINE batch_type reduce(const batch_type& a, batch_type&, batch_type&,
                                          batch_type& x) noexcept {
        batch_type k = nearbyint(constants::invlog10_2<batch_type>() * a);
        x = fnma(k, constants::log10_2hi<batch_type>(), a);
    ...
    static XSIMD_INLINE batch_type finalize(const batch_type&, const batch_type& c,
                                            const batch_type&, const batch_type&) noexcept {
        return c;
    }
};
...
template <class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> exp(batch<std::complex<T>, A> const& self,
                                           requires_arch<common>) noexcept {
 ...
 * ====================================================
 */
template <class A> static XSIMD_INLINE batch<float, A> expm1(const batch<float, A>& a) noexcept {
    using batch_type = batch<float, A>;
    batch_type k = nearbyint(constants::invlog_2<batch_type>() * a);
    ...
    using i_type = as_integer_t<batch_type>;
    i_type ik = to_int(k);
    batch_type two2mk = ::xsimd::bitwise_cast<float>((constants::maxexponent<batch_type>() - ik)
                                                     << constants::nmb<batch_type>());
    batch_type y = batch_type(1.) - two2mk - (e - x);
    return ldexp(y, ik);
}

template <class A> static XSIMD_INLINE batch<double, A> expm1(const batch<double, A>& a) noexcept {
    using batch_type = batch<double, A>;
    batch_type k = nearbyint(constants::invlog_2<batch_type>() * a);
    ...
    batch_type r = detail::horner<batch_type, 0X3FF0000000000000ULL, 0XBFA11111111110F4ULL,
                                  0X3F5A01A019FE5585ULL, 0XBF14CE199EAADBB7ULL,
                                  0X3ED0CFCA86E65239ULL, 0XBE8AFDB76E09C32DULL>(hxs);
    ...
    using i_type = as_integer_t<batch_type>;
    i_type ik = to_int(k);
    batch_type two2mk = ::xsimd::bitwise_cast<double>((constants::maxexponent<batch_type>() - ik)
                                                      << constants::nmb<batch_type>());
...
template <class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> expm1(const batch<std::complex<T>, A>& z,
                                             requires_arch<common>) noexcept {
    using batch_type = batch<std::complex<T>, A>;
...
// polar
template <class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> polar(const batch<T, A>& r, const batch<T, A>& theta,
                                             requires_arch<common>) noexcept {
...
// fdim
template <class A, class T>
XSIMD_INLINE batch<T, A> fdim(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<common>) noexcept {
...
// fmod
template <class A, class T>
XSIMD_INLINE batch<T, A> fmod(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<common>) noexcept {
 ...
 */
template <class A, class T>
XSIMD_INLINE batch<T, A> frexp(const batch<T, A>& self, batch<as_integer_t<T>, A>& exp,
                               requires_arch<common>) noexcept {
    using batch_type = batch<T, A>;
    ...
    exp = select(batch_bool_cast<typename i_type::value_type>(self != batch_type(0.)), exp,
                 i_type(typename i_type::value_type(0)));
    return select((self != batch_type(0.)),
                  x | ::xsimd::bitwise_cast<T>(constants::mask2frexp<batch_type>()),
                  batch_type(0.));
}
...
// hypot
template <class A, class T>
XSIMD_INLINE batch<T, A> hypot(batch<T, A> const& self, batch<T, A> const& other,
                               requires_arch<common>) noexcept {
 ...
 */
template <class A, class T>
XSIMD_INLINE batch<T, A> ldexp(const batch<T, A>& self, const batch<as_integer_t<T>, A>& other,
                               requires_arch<common>) noexcept {
    using batch_type = batch<T, A>;
 ...
 * ====================================================
 */
template <class A> static XSIMD_INLINE batch<float, A> gammalnB(const batch<float, A>& x) noexcept {
...
template <class A> static XSIMD_INLINE batch<float, A> gammalnC(const batch<float, A>& x) noexcept {
    return horner<batch<float, A>,
...
template <class A> static XSIMD_INLINE batch<float, A> gammaln2(const batch<float, A>& x) noexcept {
    return horner<batch<float, A>,
 ...
 * ====================================================
 */
template <class B> struct lgamma_impl;

template <class A> struct lgamma_impl<batch<float, A>> {
    using batch_type = batch<float, A>;
    static XSIMD_INLINE batch_type compute(const batch_type& a) noexcept {
        ...
        return r2;
#else
        return select(a == constants::minusinfinity<batch_type>(), constants::nan<batch_type>(),
                      select(inf_result, constants::infinity<batch_type>(), r2));
#endif
    }
...
template <class A> struct lgamma_impl<batch<double, A>> {
    using batch_type = batch<double, A>;
  ...
  private:
    ...
    static XSIMD_INLINE batch_type other(const batch_type& xx) noexcept {
        ...
        batch_type r2 =
            fma(xx - batch_type(0.5), log(xx), constants::logsqrt2pi<batch_type>() - xx);
...
XSIMD_INLINE batch<float, A> log(batch<float, A> const& self, requires_arch<common>) noexcept {
    ...
    batch_type r = fma(dk, constants::log_2hi<batch_type>(),
                       fma(s, (hfsq + R), dk * constants::log_2lo<batch_type>()) - hfsq + f);
#ifdef __FAST_MATH__
    return r;
#else
    batch_type zz = select(
        isnez,
        select(self == constants::infinity<batch_type>(), constants::infinity<batch_type>(), r),
        constants::minusinfinity<batch_type>());
...
XSIMD_INLINE batch<double, A> log(batch<double, A> const& self, requires_arch<common>) noexcept {
    ...
    x = ::xsimd::bitwise_cast<double>(hx << 32 |
                                      (i_type(0xffffffff) & ::xsimd::bitwise_cast<int_type>(x)));
    ...
    batch_type t1 = w * detail::horner<batch_type, 0x3fd999999997fa04ll, 0x3fcc71c51d8e78afll,
                                       0x3fc39a09d078c69fll>(w);
    batch_type t2 = z * detail::horner<batch_type, 0x3fe5555555555593ll, 0x3fd2492494229359ll,
...
template <class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> log(const batch<std::complex<T>, A>& z,
                                           requires_arch<common>) noexcept {
...
XSIMD_INLINE batch<double, A> log2(batch<double, A> const& self, requires_arch<common>) noexcept {
    ...
    batch_type val_lo =
        fma(lo + hi, constants::invlog_2lo<batch_type>(), lo * constants::invlog_2hi<batch_type>());
    batch_type dk = to_float(k);
...
XSIMD_INLINE batch<T, A> logN_complex_impl(const batch<T, A>& z,
                                           typename batch<T, A>::value_type base) noexcept {
    using batch_type = batch<T, A>;
...
template <class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> log2(batch<std::complex<T>, A> const& self,
                                            requires_arch<common>) noexcept {
...
XSIMD_INLINE batch<float, A> log10(batch<float, A> const& self, requires_arch<common>) noexcept {
    using batch_type = batch<float, A>;
    const batch_type ivln10hi(4.3432617188e-01f), ivln10lo(-3.1689971365e-05f),
        log10_2hi(3.0102920532e-01f), log10_2lo(7.9034151668e-07f);
    using int_type = as_integer_t<float>;
    using i_type = batch<int_type, A>;
    batch_type x = self;
    i_type k(0);
    ...
    batch_type r = fma(dk, log10_2hi,
                       fma(hibits, ivln10hi,
                           fma(lobits, ivln10hi, fma(lobits + hibits, ivln10lo, dk * log10_2lo))));
#ifndef XSIMD_NO_INFINITIES
    batch_type zz = select(
        isnez,
        select(self == constants::infinity<batch_type>(), constants::infinity<batch_type>(), r),
...
#else
    assert(all(isnez) &&
           "Calling log10 on a batch with zero value while XSIMD_NO_INFINITIES is active");
    batch_type zz = r;
#endif
#ifndef XSIMD_NO_NANS
    return select(!(self >= batch_type(0.)), constants::nan<batch_type>(), zz);
#else
    assert(all(self >= batch_type(0.)) &&
           "Calling log10 on a batch with negative value while XSIMD_NO_NANS is active");
    return zz;
#endif
}
...
XSIMD_INLINE batch<double, A> log10(batch<double, A> const& self, requires_arch<common>) noexcept {
    using batch_type = batch<double, A>;
    const batch_type ivln10hi(4.34294481878168880939e-01), ivln10lo(2.50829467116452752298e-11),
...
template <class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> log10(const batch<std::complex<T>, A>& z,
                                             requires_arch<common>) noexcept {
...
XSIMD_INLINE batch<float, A> log1p(batch<float, A> const& self, requires_arch<common>) noexcept {
    ...
    batch_type c = select(batch_bool_cast<float>(k >= i_type(2)), batch_type(1.) - (uf - self),
                          self - (uf - batch_type(1.))) /
                   uf;
    batch_type r = fma(dk, constants::log_2hi<batch_type>(),
                       fma(s, (hfsq + R), dk * constants::log_2lo<batch_type>() + c) - hfsq + f);
#ifdef __FAST_MATH__
    return r;
#else
    batch_type zz = select(
...
XSIMD_INLINE batch<double, A> log1p(batch<double, A> const& self, requires_arch<common>) noexcept {
    ...
    /* correction term ~ log(1+x)-log(u), avoid underflow in c/u */
    batch_type c = select(batch_bool_cast<double>(k >= i_type(2)), batch_type(1.) - (uf - self),
                          self - (uf - batch_type(1.))) /
                   uf;
    hu = (hu & i_type(0x000fffff)) + 0x3fe6a09e;
    batch_type f = ::xsimd::bitwise_cast<double>(
        (hu << 32) | (i_type(0xffffffff) & ::xsimd::bitwise_cast<int_type>(uf)));
    f = --f;
    batch_type hfsq = batch_type(0.5) * f * f;
    batch_type s = f / (batch_type(2.) + f);
    batch_type z = s * s;
...
template <class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> log1p(batch<std::complex<T>, A> const& self,
                                             requires_arch<common>) noexcept {
    using batch_type = batch<std::complex<T>, A>;
    using real_batch = typename batch_type::real_batch;
    batch_type u = 1 + self;
    batch_type logu = log(u);
    return select(u == batch_type(1.), self,
                  select(u.real() <= real_batch(0.), logu, logu * self / (u - batch_type(1.))));
}

// mod
template <class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch<T, A> mod(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<common>) noexcept {
    return detail::apply([](T x, T y) noexcept -> T { return x % y; }, self, other);
}
...
namespace detail {
template <class A, class T> XSIMD_INLINE batch<T, A> nearbyintf(batch<T, A> const& self) noexcept {
    using batch_type = batch<T, A>;
...
// nearbyint_as_int
template <class A>
XSIMD_INLINE batch<as_integer_t<float>, A> nearbyint_as_int(batch<float, A> const& self,
                                                            requires_arch<common>) noexcept {
    using U = as_integer_t<float>;
    return kernel::detail::apply_transform<U>(
        [](float x) noexcept -> U { return std::nearbyintf(x); }, self);
}

template <class A>
XSIMD_INLINE batch<as_integer_t<double>, A> nearbyint_as_int(batch<double, A> const& self,
                                                             requires_arch<common>) noexcept {
    using U = as_integer_t<double>;
    return kernel::detail::apply_transform<U>(
        [](double x) noexcept -> U { return std::nearbyint(x); }, self);
}

// nextafter
namespace detail {
template <class T, class A, bool is_int = std::is_integral<T>::value> struct nextafter_kernel {
    using batch_type = batch<T, A>;

    static XSIMD_INLINE batch_type next(batch_type const& b) noexcept { return b; }

    static XSIMD_INLINE batch_type prev(batch_type const& b) noexcept { return b; }
};

template <class T, class A> struct bitwise_cast_batch;

template <class A> struct bitwise_cast_batch<float, A> {
    using type = batch<int32_t, A>;
};

template <class A> struct bitwise_cast_batch<double, A> {
    using type = batch<int64_t, A>;
};

template <class T, class A> struct nextafter_kernel<T, A, false> {
    using batch_type = batch<T, A>;
...
template <class A, class T>
XSIMD_INLINE batch<T, A> nextafter(batch<T, A> const& from, batch<T, A> const& to,
                                   requires_arch<common>) noexcept {
    using kernel = detail::nextafter_kernel<T, A>;
    return select(from == to, from, select(to > from, kernel::next(from), kernel::prev(from)));
}
 ...
 */
template <class A, class T>
XSIMD_INLINE batch<T, A> pow(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<common>) noexcept {
    using batch_type = batch<T, A>;
...
template <class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> pow(const batch<std::complex<T>, A>& a,
                                           const batch<std::complex<T>, A>& z,
                                           requires_arch<common>) noexcept {
    ...
    return select(absa == ze, cplx_batch(ze),
                  cplx_batch(r * sincosTheta.second, r * sincosTheta.first));
}

template <class A, class T>
inline batch<std::complex<T>, A> pow(const batch<std::complex<T>, A>& a, const batch<T, A>& z,
                                     requires_arch<common>) noexcept {
    using cplx_batch = batch<std::complex<T>, A>;
    ...
    auto theta = z * arga;
    auto sincosTheta = xsimd::sincos(theta);
    return select(absa == 0, cplx_batch(0),
                  cplx_batch(r * sincosTheta.second, r * sincosTheta.first));
}

template <class A, class T>
inline batch<std::complex<T>, A> pow(const batch<T, A>& a, const batch<std::complex<T>, A>& z,
                                     requires_arch<common>) noexcept {
...
XSIMD_INLINE batch<T, A> reciprocal(batch<T, A> const& self, requires_arch<common>) noexcept {
    using batch_type = batch<T, A>;
...
// reduce_add
template <class A, class T>
XSIMD_INLINE std::complex<T> reduce_add(batch<std::complex<T>, A> const& self,
                                        requires_arch<common>) noexcept {
    return {reduce_add(self.real()), reduce_add(self.imag())};
}

template <class A, class T,
          class /*=typename std::enable_if<std::is_scalar<T>::value, void>::type*/>
...
namespace detail {
template <class T, T N> struct split_high {
    static constexpr T get(T i, T) { return i < N ? (i + N) : ((i % N) + N); }
};
...
XSIMD_INLINE T reduce(Op op, batch<T, A> const& self,
                      std::integral_constant<unsigned, Lvl>) noexcept {
    using index_type = as_unsigned_integer_t<T>;
    batch<T, A> split =
        swizzle(self, make_batch_constant<index_type, split_high<index_type, Lvl / 2>, A>());
...
XSIMD_INLINE T reduce_max(batch<T, A> const& self, requires_arch<common>) noexcept {
    return detail::reduce([](batch<T, A> const& x, batch<T, A> const& y) { return max(x, y); },
...
XSIMD_INLINE T reduce_min(batch<T, A> const& self, requires_arch<common>) noexcept {
    return detail::reduce([](batch<T, A> const& x, batch<T, A> const& y) { return min(x, y); },
                          self, std::integral_constant<unsigned, batch<T, A>::size>());
}

// reduce_mul
template <class A, class T>
XSIMD_INLINE std::complex<T> reduce_mul(batch<std::complex<T>, A> const& self,
                                        requires_arch<common>) noexcept {
...
// remainder
template <class A>
XSIMD_INLINE batch<float, A> remainder(batch<float, A> const& self, batch<float, A> const& other,
                                       requires_arch<common>) noexcept {
    return fnma(nearbyint(self / other), other, self);
}
template <class A>
XSIMD_INLINE batch<double, A> remainder(batch<double, A> const& self, batch<double, A> const& other,
                                        requires_arch<common>) noexcept {
    return fnma(nearbyint(self / other), other, self);
}
template <class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch<T, A> remainder(batch<T, A> const& self, batch<T, A> const& other,
                                   requires_arch<common>) noexcept {
    auto mod = self % other;
...
select(batch_bool<T, A> const& cond, batch<std::complex<T>, A> const& true_br,
       batch<std::complex<T>, A> const& false_br, requires_arch<common>) noexcept {
    return {select(cond, true_br.real(), false_br.real()),
            select(cond, true_br.imag(), false_br.imag())};
}
...
XSIMD_INLINE batch<T, A> sign(batch<T, A> const& self, requires_arch<common>) noexcept {
    using batch_type = batch<T, A>;
    batch_type res = select(self > batch_type(0), batch_type(1), batch_type(0)) -
                     select(self < batch_type(0), batch_type(1), batch_type(0));
    return res;
}

namespace detail {
template <class T, class A> XSIMD_INLINE batch<T, A> signf(batch<T, A> const& self) noexcept {
    using batch_type = batch<T, A>;
    batch_type res = select(self > batch_type(0.f), batch_type(1.f), batch_type(0.f)) -
                     select(self < batch_type(0.f), batch_type(1.f), batch_type(0.f));
#ifdef XSIMD_NO_NANS
    return res;
#else
...
template <class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> sign(const batch<std::complex<T>, A>& z,
                                            requires_arch<common>) noexcept {
    using batch_type = batch<std::complex<T>, A>;
    using real_batch = typename batch_type::real_batch;
    auto rz = z.real();
    auto iz = z.imag();
    return select(rz != real_batch(0.), batch_type(sign(rz)), batch_type(sign(iz)));
}
...
namespace detail {
template <class T, class A> XSIMD_INLINE batch<T, A> signnzf(batch<T, A> const& self) noexcept {
    using batch_type = batch<T, A>;
#ifndef XSIMD_NO_NANS
    return select(isnan(self), constants::nan<batch_type>(),
                  batch_type(1.) | (constants::signmask<batch_type>() & self));
#else
...
// sqrt
template <class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> sqrt(batch<std::complex<T>, A> const& z,
                                            requires_arch<common>) noexcept {

    constexpr T csqrt_scale_factor =
        std::is_same<T, float>::value ? 6.7108864e7f : 1.8014398509481984e16;
    constexpr T csqrt_scale =
        std::is_same<T, float>::value ? 1.220703125e-4f : 7.450580596923828125e-9;
    using batch_type = batch<std::complex<T>, A>;
    ...
    return select(
        y == ze,
        select(x == ze, batch_type(ze, ze),
               select(x < ze, batch_type(ze, sqrt_x), batch_type(sqrt_x, ze))),
        select(x == ze, select(y > ze, batch_type(sqrt_hy, sqrt_hy), batch_type(sqrt_hy, -sqrt_hy)),
               resg));
}
 ...
 * ====================================================
 */
template <class B> struct stirling_kernel;

template <class A> struct stirling_kernel<batch<float, A>> {
    using batch_type = batch<float, A>;
    static XSIMD_INLINE batch_type compute(const batch_type& x) noexcept {
        return horner<batch_type, 0x3daaaaab, 0x3b638e39, 0xbb2fb930, 0xb970b359>(x);
    }
...
template <class A> struct stirling_kernel<batch<double, A>> {
    using batch_type = batch<double, A>;
    static XSIMD_INLINE batch_type compute(const batch_type& x) noexcept {
 ...
 * ====================================================
 */
template <class T, class A> XSIMD_INLINE batch<T, A> stirling(const batch<T, A>& a) noexcept {
    using batch_type = batch<T, A>;
 ...
 * ====================================================
 */
template <class B> struct tgamma_kernel;

template <class A> struct tgamma_kernel<batch<float, A>> {
    using batch_type = batch<float, A>;
    static XSIMD_INLINE batch_type compute(const batch_type& x) noexcept {
        return horner<batch_type,
...
template <class A> struct tgamma_kernel<batch<double, A>> {
    using batch_type = batch<double, A>;
    static XSIMD_INLINE batch_type compute(const batch_type& x) noexcept {
        return horner<batch_type,
 ...
 * ====================================================
 */
template <class B> XSIMD_INLINE B tgamma_large_negative(const B& a) noexcept {
    B st = stirling(a);
    B p = floor(a);
...
template <class B, class BB> XSIMD_INLINE B tgamma_other(const B& a, const BB& test) noexcept {
...
XSIMD_INLINE batch<T, A> tgamma(batch<T, A> const& self, requires_arch<common>) noexcept {
    ...
    return r2;
#else
    return select(self == batch_type(0.), copysign(constants::infinity<batch_type>(), self),
                  select(nan_result, constants::nan<batch_type>(), r2));
#endif
}
