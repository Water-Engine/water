...
#define XSIMD_DEFINE_CONSTANT(NAME, SINGLE, DOUBLE)                                                \
    template <class T> XSIMD_INLINE T NAME() noexcept {                                            \
        return T(NAME<typename T::value_type>());                                                  \
    }                                                                                              \
    template <> XSIMD_INLINE float NAME<float>() noexcept { return SINGLE; }                       \
    template <> XSIMD_INLINE double NAME<double>() noexcept { return DOUBLE; }

#define XSIMD_DEFINE_CONSTANT_HEX(NAME, SINGLE, DOUBLE)                                            \
    template <class T> XSIMD_INLINE T NAME() noexcept {                                            \
        return T(NAME<typename T::value_type>());                                                  \
    }                                                                                              \
    template <> XSIMD_INLINE float NAME<float>() noexcept {                                        \
        return bit_cast<float>((uint32_t)SINGLE);                                                  \
    }                                                                                              \
    template <> XSIMD_INLINE double NAME<double>() noexcept {                                      \
...
XSIMD_DEFINE_CONSTANT(infinity, (std::numeric_limits<float>::infinity()),
...
XSIMD_DEFINE_CONSTANT(smallestposval, std::numeric_limits<float>::min(),
...
template <class T> constexpr T allbits() noexcept;

template <class T> constexpr as_integer_t<T> mask1frexp() noexcept;

template <class T> constexpr as_integer_t<T> mask2frexp() noexcept;

template <class T> constexpr as_integer_t<T> maxexponent() noexcept;

template <class T> constexpr as_integer_t<T> maxexponentm1() noexcept;

template <class T> constexpr int32_t nmb() noexcept;

template <class T> constexpr T zero() noexcept;

template <class T> constexpr T minvalue() noexcept;

template <class T> constexpr T maxvalue() noexcept;
...
namespace detail {
template <class T, bool = std::is_integral<T>::value> struct allbits_impl {
    static constexpr T get_value() noexcept { return T(~0); }
};

template <class T> struct allbits_impl<T, false> {
    static constexpr T get_value() noexcept { return nan<T>(); }
};
} // namespace detail

template <class T> XSIMD_INLINE constexpr T allbits() noexcept {
...
template <class T> XSIMD_INLINE constexpr as_integer_t<T> mask1frexp() noexcept {
    return as_integer_t<T>(mask1frexp<typename T::value_type>());
}

template <> XSIMD_INLINE constexpr int32_t mask1frexp<float>() noexcept { return 0x7f800000; }

template <> XSIMD_INLINE constexpr int64_t mask1frexp<double>() noexcept {
...
template <class T> XSIMD_INLINE constexpr as_integer_t<T> mask2frexp() noexcept {
    return as_integer_t<T>(mask2frexp<typename T::value_type>());
}

template <> XSIMD_INLINE constexpr int32_t mask2frexp<float>() noexcept { return 0x3f000000; }

template <> XSIMD_INLINE constexpr int64_t mask2frexp<double>() noexcept {
...
template <class T> XSIMD_INLINE constexpr as_integer_t<T> maxexponent() noexcept {
    return as_integer_t<T>(maxexponent<typename T::value_type>());
}

template <> XSIMD_INLINE constexpr int32_t maxexponent<float>() noexcept { return 127; }

template <> XSIMD_INLINE constexpr int64_t maxexponent<double>() noexcept { return 1023; }

/******************************
 * maxexponent implementation *
 ******************************/

template <class T> XSIMD_INLINE constexpr as_integer_t<T> maxexponentm1() noexcept {
    return as_integer_t<T>(maxexponentm1<typename T::value_type>());
}

template <> XSIMD_INLINE constexpr int32_t maxexponentm1<float>() noexcept { return 126; }

template <> XSIMD_INLINE constexpr int64_t maxexponentm1<double>() noexcept { return 1022; }

/**********************
 * nmb implementation *
 **********************/

template <class T> XSIMD_INLINE constexpr int32_t nmb() noexcept {
    return nmb<typename T::value_type>();
}

template <> XSIMD_INLINE constexpr int32_t nmb<float>() noexcept { return 23; }

template <> XSIMD_INLINE constexpr int32_t nmb<double>() noexcept { return 52; }

/***********************
 * zero implementation *
 ***********************/

template <class T> XSIMD_INLINE constexpr T zero() noexcept { return T(typename T::value_type(0)); }
...
namespace detail {
template <class T> struct minvalue_impl {
    static constexpr T get_value() noexcept {
        return std::numeric_limits<typename T::value_type>::min();
    }
};

template <class T> struct minvalue_common {
    static constexpr T get_value() noexcept { return std::numeric_limits<T>::min(); }
};

template <> struct minvalue_impl<int8_t> : minvalue_common<int8_t> {};
template <> struct minvalue_impl<uint8_t> : minvalue_common<uint8_t> {};
template <> struct minvalue_impl<int16_t> : minvalue_common<int16_t> {};
template <> struct minvalue_impl<uint16_t> : minvalue_common<uint16_t> {};
template <> struct minvalue_impl<int32_t> : minvalue_common<int32_t> {};
template <> struct minvalue_impl<uint32_t> : minvalue_common<uint32_t> {};
template <> struct minvalue_impl<int64_t> : minvalue_common<int64_t> {};
template <> struct minvalue_impl<uint64_t> : minvalue_common<uint64_t> {};

template <> struct minvalue_impl<float> {
    XSIMD_INLINE static float get_value() noexcept { return bit_cast<float>((uint32_t)0xff7fffff); }
};

template <> struct minvalue_impl<double> {
