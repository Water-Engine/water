 ...
 */
template <size_t Width> struct rvv : xsimd::common {
...
#define XSIMD_RVV_MAKE_TYPE(scalar, t, s, vmul)                                                    \
    template <> struct rvv_type_info<scalar, rvv_width_m1 * vmul> {                                \
        ...
        template <class U> static XSIMD_INLINE type bitcast(U x) noexcept {                        \
        ...
        template <> XSIMD_INLINE type bitcast<type>(type x) noexcept { return x; }                 \
        template <class U> static XSIMD_INLINE byte_type as_bytes(U x) noexcept {                  \
...
rvv_type_info<int8_t, rvv_width_m1 * 8>::bitcast<__rvv_uint8m8_t>(__rvv_uint8m8_t x) noexcept {
    return __riscv_vreinterpret_i8m8(x);
}
template <>
XSIMD_INLINE rvv_type_info<int8_t, rvv_width_m1 * 1>::type
rvv_type_info<int8_t, rvv_width_m1 * 1>::bitcast<__rvv_uint8m1_t>(__rvv_uint8m1_t x) noexcept {
    return __riscv_vreinterpret_i8m1(x);
}
template <>
XSIMD_INLINE rvv_type_info<uint16_t, rvv_width_m1 * 1>::type
rvv_type_info<uint16_t, rvv_width_m1 * 1>::bitcast<__rvv_uint8m1_t>(__rvv_uint8m1_t x) noexcept {
    return __riscv_vreinterpret_u16m1(x);
}
template <>
XSIMD_INLINE rvv_type_info<uint32_t, rvv_width_m1 * 1>::type
rvv_type_info<uint32_t, rvv_width_m1 * 1>::bitcast<__rvv_uint8m1_t>(__rvv_uint8m1_t x) noexcept {
    return __riscv_vreinterpret_u32m1(x);
}
template <>
XSIMD_INLINE rvv_type_info<uint64_t, rvv_width_m1 * 1>::type
rvv_type_info<uint64_t, rvv_width_m1 * 1>::bitcast<__rvv_uint8m1_t>(__rvv_uint8m1_t x) noexcept {
...
template <class T, size_t Width> struct rvv_blob : public rvv_type_info<T, Width> {
...
template <class T, size_t divisor> struct rvv_semiblob : public rvv_type_info<T, rvv_width_m1> {
    ...
    fixed_type value;
    template <size_t div> vuint8m1_t get_bytes() const;
    template <> vuint8m1_t get_bytes<2>() const { return __riscv_vlmul_ext_v_u8mf2_u8m1(value); }
    template <> vuint8m1_t get_bytes<4>() const { return __riscv_vlmul_ext_v_u8mf4_u8m1(value); }
    template <> vuint8m1_t get_bytes<8>() const { return __riscv_vlmul_ext_v_u8mf8_u8m1(value); }
    type get() const noexcept {
        vuint8m1_t bytes = get_bytes<divisor>();
        return bitcast(bytes);
    }
    template <size_t div> void set_bytes(vuint8m1_t);
    template <> void set_bytes<2>(vuint8m1_t v) { value = __riscv_vlmul_trunc_v_u8m1_u8mf2(v); }
    template <> void set_bytes<4>(vuint8m1_t v) { value = __riscv_vlmul_trunc_v_u8m1_u8mf4(v); }
    template <> void set_bytes<8>(vuint8m1_t v) { value = __riscv_vlmul_trunc_v_u8m1_u8mf8(v); }
    void set(type v) {
        vuint8m1_t bytes = as_bytes(v);
        set_bytes<divisor>(bytes);
    }
};
template <class T> struct rvv_blob<T, rvv_width_mf2> : rvv_semiblob<T, 2> {};
template <class T> struct rvv_blob<T, rvv_width_mf4> : rvv_semiblob<T, 4> {};
template <class T> struct rvv_blob<T, rvv_width_mf8> : rvv_semiblob<T, 8> {};
...
template <class T>
using rvv_fix_char_t =
    typename std::conditional<std::is_same<char, typename std::decay<T>::type>::value, rvv_char_t,
                              T>::type;
...
// type).
//
template <class T, size_t Width> struct rvv_reg {
    static constexpr size_t width = Width;
    ...
    byte_type get_bytes() const noexcept { return blob_type::as_bytes(value.get()); }
    operator register_type() const noexcept { return value.get(); }
};
template <class T, size_t Width = XSIMD_RVV_BITS>
using rvv_reg_t = typename std::conditional<!std::is_void<T>::value,
                                            rvv_reg<rvv_fix_char_t<T>, Width>, void>::type;
...
#define XSIMD_RVV_MAKE_BOOL_TYPE(i)                                                                \
    template <> struct rvv_bool_info<i> {                                                          \
        ...
        template <class T> static XSIMD_INLINE type bitcast(T value) noexcept {                    \
...
template <class T, size_t Width> struct rvv_bool {
    ...
    storage_type value;
    rvv_bool() = default;
    rvv_bool(type v) noexcept : value(__riscv_vreinterpret_u8m1(v)) {}
    template <class U, typename std::enable_if<sizeof(T) == sizeof(U), int>::type = 0>
    rvv_bool(rvv_bool<U, Width> v) : value(v.value) {}
...
template <class T>
using floating_point_rvv_vector_type =
    typename rvv_vector_type_impl<8 * sizeof(T)>::floating_point_type;

template <class T>
using signed_int_or_floating_point_rvv_vector_type =
    typename std::conditional<std::is_floating_point<T>::value, floating_point_rvv_vector_type<T>,
...
XSIMD_DECLARE_SIMD_REGISTER(unsigned long long int, rvv,
                            detail::rvv_vector_type<unsigned long long int>);
...
namespace detail {
template <class T> struct rvv_bool_simd_register {
...
template <class T> struct get_bool_simd_register<T, rvv> {
