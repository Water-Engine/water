...
def parse_benchmark_file(file_path):
    try:
        ...
        with open(file_path, "r") as f:
            for line in f:
                if "avg nps" in line and "fen" in line:
                    match = re.search(r"avg nps:\s*(\d+)\s*\|\s*fen:\s*(.*)", line)
                    if match:
                        nps = int(match.group(1))
                        fen = match.group(2).strip()
                        records.append({"fen": fen, "nps": nps})
...
if __name__ == "__main__":
    ...
    # Merge the datasets on the 'fen' column to find corresponding runs
    merged_df = pd.merge(df1, df2, on="fen")

    if merged_df.empty:
        print("No matching FEN strings found. Cannot perform a paired t-test.")
        sys.exit(1)

    print(f"\nFound {len(merged_df)} matching FENs between the two files.")
