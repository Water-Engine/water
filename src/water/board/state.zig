...
const castling_keys: [16]u64 = blk: {
    ...
    for (0..16) |i| {
        array[i] = generateCastlingKey(i);
    }

    break :blk array;
};

fn generateCastlingKey(index: usize) u64 {
    const random_offset: usize = 768;
...
test "Zobrist" {
    ...
    try expectEqual(0x4AE7D6A36EB5DBCB, Zobrist.piece(.black_king, .a5));
    try expectEqual(0x1C99DED33CB890A1, Zobrist.enPassant(.fd));

    const castling_expecteds: [16]u64 = .{
        0x0000000000000000, 0x31D71DCE64B2C310, 0xF165B587DF898190, 0xC0B2A849BB3B4280,
        0xA57E6339DD2CF3A0, 0x94A97EF7B99E30B0, 0x541BD6BE02A57230, 0x65CCCB706617B120,
        0x1EF6E6DBB1961EC9, 0x2F21FB15D524DDD9, 0xEF93535C6E1F9F59, 0xDE444E920AAD5C49,
        0xBB8885E26CBAED69, 0x8A5F982C08082E79, 0x4AED3065B3336CF9, 0x7B3A2DABD781AFE9,
    };

    for (castling_expecteds, 0..) |ce, i| {
        try expectEqual(ce, Zobrist.castling(i));
    }

    const castling_idx_expecteds: [4]u64 = .{
        0x31D71DCE64B2C310, 0xF165B587DF898190, 0xA57E6339DD2CF3A0, 0x1EF6E6DBB1961EC9,
    };

    for (castling_idx_expecteds, 0..) |cie, i| {
        try expectEqual(cie, Zobrist.castlingIdx(i));
    }
