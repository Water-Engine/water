...
XSIMD_INLINE batch<T, A> abs(batch<T, A> const& self, requires_arch<ssse3>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_abs_epi8(self); }
...
XSIMD_INLINE batch<T, A> extract_pair(batch<T, A> const&, batch<T, A> const& other, std::size_t,
...
template <class A, class T,
          class _ = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch<T, A> extract_pair(batch<T, A> const& self, batch<T, A> const& other,
                                      std::size_t i, requires_arch<ssse3>) noexcept {
...
template <class A>
XSIMD_INLINE batch<uint8_t, A> swizzle(batch<uint8_t, A> const& self, batch<uint8_t, A> mask,
                                       requires_arch<ssse3>) noexcept {
    return _mm_shuffle_epi8(self, mask);
}
template <class A>
XSIMD_INLINE batch<int8_t, A> swizzle(batch<int8_t, A> const& self, batch<uint8_t, A> mask,
                                      requires_arch<ssse3>) noexcept {
    return _mm_shuffle_epi8(self, mask);
}
...
swizzle(batch<T, A> const& self, batch<IT, A> mask, requires_arch<ssse3>) noexcept {
    ...
    return bitwise_cast<T>(
        swizzle(bitwise_cast<uint8_t>(self), bitwise_cast<uint8_t>(mask * comb + pikes), ssse3{}));
}
...
XSIMD_INLINE batch<uint16_t, A> swizzle(batch<uint16_t, A> const& self,
                                        ...
                                        requires_arch<ssse3>) noexcept {
    constexpr batch_constant<uint8_t, A, 2 * V0, 2 * V0 + 1, 2 * V1, 2 * V1 + 1, 2 * V2, 2 * V2 + 1,
                             2 * V3, 2 * V3 + 1, 2 * V4, 2 * V4 + 1, 2 * V5, 2 * V5 + 1, 2 * V6,
                             2 * V6 + 1, 2 * V7, 2 * V7 + 1>
        mask8;
