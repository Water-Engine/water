...
XSIMD_INLINE batch<double, A> fast_cast(batch<int64_t, A> const& x, batch<double, A> const&,
    ...
    __m128i xL =
        _mm_blend_epi16(x, _mm_castpd_si128(_mm_set1_pd(0x0010000000000000)), 0x88); //  2^52
    __m128d f =
...
XSIMD_INLINE batch<double, A> fast_cast(batch<uint64_t, A> const& x, batch<double, A> const&,
    ...
    __m128i xL =
        _mm_blend_epi16(x, _mm_castpd_si128(_mm_set1_pd(0x0010000000000000)), 0xcc); //  2^52
    __m128d f =
...
XSIMD_INLINE batch_bool<T, A> eq(batch<T, A> const& self, batch<T, A> const& other,
                                 requires_arch<sse4_1>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 8) { return _mm_cmpeq_epi64(self, other); }
    else {
...
XSIMD_INLINE batch<T, A> insert(batch<T, A> const& self, T val, index<I> pos,
                                requires_arch<sse4_1>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_insert_epi8(self, val, I); }
...
XSIMD_INLINE batch<T, A> max(batch<T, A> const& self, batch<T, A> const& other,
    ...
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_max_epi8(self, other); }
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_max_epu8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> min(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<sse4_1>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_min_epi8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_min_epu8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> mul(batch<T, A> const& self, batch<T, A> const& other,
    ...
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
        return _mm_or_si128(
            _mm_and_si128(_mm_mullo_epi16(self, other),
                          _mm_srli_epi16(_mm_cmpeq_epi8(self, self), 8)),
...
// select
namespace detail {
template <class T> XSIMD_INLINE constexpr T interleave(T const& cond) noexcept {
    return (((cond * 0x0101010101010101ULL & 0x8040201008040201ULL) * 0x0102040810204081ULL >> 49) &
            0x5555) |
           (((cond * 0x0101010101010101ULL & 0x8040201008040201ULL) * 0x0102040810204081ULL >> 48) &
            0xAAAA);
}
} // namespace detail
...
XSIMD_INLINE batch<T, A> select(batch_bool<T, A> const& cond, batch<T, A> const& true_br,
                                batch<T, A> const& false_br, requires_arch<sse4_1>) noexcept {
...
select(batch_bool<double, A> const& cond, batch<double, A> const& true_br,
       batch<double, A> const& false_br, requires_arch<sse4_1>) noexcept {
...
XSIMD_INLINE batch<T, A> select(batch_bool_constant<T, A, Values...> const&,
    ...
    XSIMD_IF_CONSTEXPR(sizeof(T) == 2) { return _mm_blend_epi16(false_br, true_br, mask); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 4) {
