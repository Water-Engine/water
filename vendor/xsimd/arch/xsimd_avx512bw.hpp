...
XSIMD_INLINE batch_bool<T, A> compare_int_avx512bw(batch<T, A> const& self,
...
XSIMD_INLINE batch<T, A> abs(batch<T, A> const& self, requires_arch<avx512bw>) noexcept {
    ...
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_abs_epi8(self); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> add(batch<T, A> const& self, batch<T, A> const& other,
    ...
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_add_epi8(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> avgr(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<avx512bw>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_avg_epu8(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
template <class A, class T,
          class = typename std::enable_if<batch_bool<T, A>::size == 64, void>::type>
...
template <class A, class T,
          class = typename std::enable_if<batch_bool<T, A>::size == 64, void>::type>
XSIMD_INLINE batch_bool<T, A> load_aligned(bool const* mem, batch_bool<T, A>,
                                           requires_arch<avx512bw>) noexcept {
...
XSIMD_INLINE batch<T, A> max(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<avx512bw>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_max_epi8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_max_epu8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> min(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<avx512bw>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_min_epi8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_min_epu8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> mul(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<avx512bw>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
        __m512i upper = _mm512_and_si512(_mm512_mullo_epi16(self, other),
                                         _mm512_srli_epi16(_mm512_set1_epi16(-1), 8));
        __m512i lower = _mm512_slli_epi16(
            _mm512_mullo_epi16(_mm512_srli_epi16(self, 8), _mm512_srli_epi16(other, 8)), 8);
...
XSIMD_INLINE batch<T, A> sadd(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<avx512bw>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_adds_epi8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_adds_epu8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> select(batch_bool<T, A> const& cond, batch<T, A> const& true_br,
                                batch<T, A> const& false_br, requires_arch<avx512bw>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
...
template <size_t N, class A, class T,
          class = typename std::enable_if<(N & 3) == 2 && (N < 64)>::type>
...
XSIMD_INLINE batch<T, A> slide_right(batch<T, A> const& x, requires_arch<avx512bw>) noexcept {
    ...
    __mmask32 mask = 0xFFFFFFFFu >> ((N / 2) & 31);
    auto slide_pattern =
        make_batch_constant<uint16_t, detail::make_slide_right_pattern<N / 2>, A>();
    return _mm512_maskz_permutexvar_epi16(mask, slide_pattern.as_batch(), x);
}
...
XSIMD_INLINE batch<T, A> ssub(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<avx512bw>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_subs_epi8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_subs_epu8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> sub(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<avx512bw>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm512_sub_epi8(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
template <class A>
XSIMD_INLINE batch<uint16_t, A> swizzle(batch<uint16_t, A> const& self, batch<uint16_t, A> mask,
                                        requires_arch<avx512bw>) noexcept {
    return _mm512_permutexvar_epi16(mask, self);
}

template <class A>
XSIMD_INLINE batch<int16_t, A> swizzle(batch<int16_t, A> const& self, batch<uint16_t, A> mask,
                                       requires_arch<avx512bw>) noexcept {
