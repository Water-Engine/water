...
#define builtin_popcount(v) __builtin_popcountg(v)
        ...
        XSIMD_IF_CONSTEXPR(batch_bool<T, A>::size <= 32) {
            ...
            m = (m & (uint64_t)~(uint64_t)0 / 15 * 3) +
                ((m >> 2) & (uint64_t)~(uint64_t)0 / 15 * 3);       // temp
            m = (m + (m >> 4)) & (uint64_t)~(uint64_t)0 / 255 * 15; // temp
            return (m * ((uint64_t)~(uint64_t)0 / 255)) >>
...
template <class A>
XSIMD_INLINE batch_bool<float, A> isinf(batch<float, A> const& self,
                                        requires_arch<common>) noexcept {
#ifdef __FAST_MATH__
    (void)self;
    return {false};
...
template <class A>
XSIMD_INLINE batch_bool<double, A> isinf(batch<double, A> const& self,
                                         requires_arch<common>) noexcept {
#ifdef __FAST_MATH__
    (void)self;
    return {false};
...
template <class A>
XSIMD_INLINE batch_bool<float, A> isfinite(batch<float, A> const& self,
                                           requires_arch<common>) noexcept {
    return (self - self) == 0.f;
}
template <class A>
XSIMD_INLINE batch_bool<double, A> isfinite(batch<double, A> const& self,
                                            requires_arch<common>) noexcept {
    return (self - self) == 0.;
}
...
// logical_and
template <class A, class T>
XSIMD_INLINE batch<T, A> logical_and(batch<T, A> const& self, batch<T, A> const& other,
                                     requires_arch<common>) noexcept {
...
// logical_or
template <class A, class T>
XSIMD_INLINE batch<T, A> logical_or(batch<T, A> const& self, batch<T, A> const& other,
                                    requires_arch<common>) noexcept {
    return detail::apply([](T x, T y) noexcept { return x || y; }, self, other);
}
