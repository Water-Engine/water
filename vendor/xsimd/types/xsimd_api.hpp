...
XSIMD_INLINE batch_bool<T_out, A> batch_bool_cast(batch_bool<T_in, A> const& x) noexcept {
    ...
    static_assert(batch_bool<T_out, A>::size == batch_bool<T_in, A>::size,
                  "Casting between incompatibles batch_bool types.");
 ...
 */
template <class T, class A>
XSIMD_INLINE auto bitwise_and(batch<T, A> const& x, batch<T, A> const& y) noexcept
    -> decltype(x & y) {
    detail::static_check_supported_config<T, A>();
    return x & y;
}
 ...
 */
template <class T, class A>
XSIMD_INLINE auto bitwise_and(batch_bool<T, A> const& x, batch_bool<T, A> const& y) noexcept
    -> decltype(x & y) {
    detail::static_check_supported_config<T, A>();
    return x & y;
}
 ...
 */
template <class T, class A>
XSIMD_INLINE batch_bool<T, A> bitwise_andnot(batch_bool<T, A> const& x,
                                             batch_bool<T, A> const& y) noexcept {
    detail::static_check_supported_config<T, A>();
    return kernel::bitwise_andnot<A>(x, y, A{});
}
 ...
 */
template <class T, class A> XSIMD_INLINE batch<T, A> bitwise_not(batch<T, A> const& x) noexcept {
    detail::static_check_supported_config<T, A>();
 ...
 */
template <class T, class A>
XSIMD_INLINE auto bitwise_or(batch<T, A> const& x, batch<T, A> const& y) noexcept
    -> decltype(x | y) {
    detail::static_check_supported_config<T, A>();
    return x | y;
}
 ...
 */
template <class T, class A>
XSIMD_INLINE auto bitwise_or(batch_bool<T, A> const& x, batch_bool<T, A> const& y) noexcept
    -> decltype(x | y) {
    detail::static_check_supported_config<T, A>();
    return x | y;
}
 ...
 */
template <class T, class A>
XSIMD_INLINE auto bitwise_xor(batch<T, A> const& x, batch<T, A> const& y) noexcept
    -> decltype(x ^ y) {
    detail::static_check_supported_config<T, A>();
    return x ^ y;
}
 ...
 */
template <class T, class A>
XSIMD_INLINE auto bitwise_xor(batch_bool<T, A> const& x, batch_bool<T, A> const& y) noexcept
    -> decltype(x ^ y) {
    detail::static_check_supported_config<T, A>();
    return x ^ y;
}
...
XSIMD_INLINE simd_return_type<From, To, A> broadcast_as(From v) noexcept {
    ...
    using value_type =
        typename std::conditional<std::is_same<From, bool>::value, bool, batch_value_type>::type;
 ...
 */
template <class T, class A>
XSIMD_INLINE batch<T, A> clip(batch<T, A> const& x, batch<T, A> const& lo,
                              batch<T, A> const& hi) noexcept {
    detail::static_check_supported_config<T, A>();
 ...
 */
template <class T, class A> XSIMD_INLINE size_t count(batch_bool<T, A> const& x) noexcept {
    detail::static_check_supported_config<T, A>();
 ...
 */
template <class T, class A> XSIMD_INLINE batch<T, A> decr(batch<T, A> const& x) noexcept {
    detail::static_check_supported_config<T, A>();
 ...
 */
template <class T, class A>
XSIMD_INLINE auto eq(batch_bool<T, A> const& x, batch_bool<T, A> const& y) noexcept
    -> decltype(x == y) {
    detail::static_check_supported_config<T, A>();
    return x == y;
}
 ...
 */
template <class T, class A>
XSIMD_INLINE batch<T, A> extract_pair(batch<T, A> const& x, batch<T, A> const& y,
 ...
 */
template <class T, class A>
XSIMD_INLINE batch<T, A> fnma(batch<T, A> const& x, batch<T, A> const& y,
                              batch<T, A> const& z) noexcept {
    detail::static_check_supported_config<T, A>();
 ...
 */
template <class T, class A>
XSIMD_INLINE batch<T, A> fnms(batch<T, A> const& x, batch<T, A> const& y,
                              batch<T, A> const& z) noexcept {
    detail::static_check_supported_config<T, A>();
 ...
 * @param y a batch of integer or floating point values.
 * @param z a batch of integer or floating point values.
 * @return  a batch where each even-indexed element is computed as <tt>x * y - z</tt> and each
 * odd-indexed element as <tt>x * y + z</tt>
 */
template <class T, class A>
 ...
 */
template <class T, class A> XSIMD_INLINE batch<T, A> incr(batch<T, A> const& x) noexcept {
    detail::static_check_supported_config<T, A>();
 ...
 */
template <class B> XSIMD_INLINE B infinity() {
...
template <class To, class A = default_arch, class From>
XSIMD_INLINE simd_return_type<std::complex<From>, To, A> load_as(std::complex<From> const* ptr,
...
XSIMD_INLINE simd_return_type<xtl::xcomplex<From, From, i3ec>, To, A>
load_as(xtl::xcomplex<From, From, i3ec> const* ptr, aligned_mode) noexcept {
    detail::static_check_supported_config<To, A>();
    detail::static_check_supported_config<From, A>();
...
template <class To, class A = default_arch, class From, bool i3ec>
XSIMD_INLINE simd_return_type<xtl::xcomplex<From, From, i3ec>, To, A>
load_as(xtl::xcomplex<From, From, i3ec> const* ptr, unaligned_mode) noexcept {
    detail::static_check_supported_config<To, A>();
    detail::static_check_supported_config<From, A>();
 ...
 * @return a batch of positive infinity
 */
template <class B> XSIMD_INLINE B minusinfinity() noexcept {
    using T = typename B::value_type;
    using A = typename B::arch_type;
    detail::static_check_supported_config<T, A>();
    return B(-std::numeric_limits<T>::infinity());
 ...
 */
template <class T, class A>
XSIMD_INLINE batch<as_integer_t<T>, A> nearbyint_as_int(batch<T, A> const& x) noexcept {
    detail::static_check_supported_config<T, A>();
 ...
 */
template <class T, class A>
XSIMD_INLINE auto neq(batch_bool<T, A> const& x, batch_bool<T, A> const& y) noexcept
    -> decltype(x != y) {
    detail::static_check_supported_config<T, A>();
    return x != y;
}
 ...
 */
template <class T, class A> XSIMD_INLINE batch<T, A> neg(batch<T, A> const& x) noexcept {
    detail::static_check_supported_config<T, A>();
    return -x;
}
 ...
 */
template <class T, class A>
XSIMD_INLINE complex_batch_type_t<batch<T, A>>
polar(batch<T, A> const& r, batch<T, A> const& theta = batch<T, A>{}) noexcept {
    detail::static_check_supported_config<T, A>();
 ...
 * @return \c x.
 */
template <class T, class A> XSIMD_INLINE batch<T, A> pos(batch<T, A> const& x) noexcept {
    detail::static_check_supported_config<T, A>();
    return +x;
}
 ...
 */
template <class T, class A>
XSIMD_INLINE batch<std::complex<T>, A> pow(batch<std::complex<T>, A> const& x,
                                           batch<T, A> const& y) noexcept {
    detail::static_check_supported_config<T, A>();
    return kernel::pow<A>(x, y, A{});
}
 ...
 */
template <class T, class A>
XSIMD_INLINE batch<std::complex<T>, A> pow(batch<T, A> const& x,
                                           batch<std::complex<T>, A> const& y) noexcept {
    detail::static_check_supported_config<T, A>();
    return kernel::pow<A>(x, y, A{});
}
...
template <class T, class ITy, class A,
          class = typename std::enable_if<std::is_integral<ITy>::value, void>::type>
...
template <class T, class A, class F> XSIMD_INLINE T reduce(F&& f, batch<T, A> const& x) noexcept {
    detail::static_check_supported_config<T, A>();
    return kernel::detail::reduce(std::forward<F>(f), x,
                                  std::integral_constant<unsigned, batch<T, A>::size>());
}
 ...
 * @return the result of the reduction.
 */
template <class T, class A> XSIMD_INLINE T reduce_mul(batch<T, A> const& x) noexcept {
    detail::static_check_supported_config<T, A>();
...
XSIMD_INLINE batch<T, A> select(batch_bool<T, A> const& cond, batch<T, A> const& true_br,
                                batch<T, A> const& false_br) noexcept {
    detail::static_check_supported_config<T, A>();
 ...
 */
template <class T, class A>
XSIMD_INLINE batch<std::complex<T>, A> select(batch_bool<T, A> const& cond,
                                              batch<std::complex<T>, A> const& true_br,
                                              batch<std::complex<T>, A> const& false_br) noexcept {
    detail::static_check_supported_config<T, A>();
    return kernel::select<A>(cond, true_br, false_br, A{});
}
...
XSIMD_INLINE batch<T, A> select(batch_bool_constant<T, A, Values...> const& cond,
                                batch<T, A> const& true_br, batch<T, A> const& false_br) noexcept {
    detail::static_check_supported_config<T, A>();
    return kernel::select<A>(cond, true_br, false_br, A{});
}
...
shuffle(batch<T, A> const& x, batch<T, A> const& y,
        batch_constant<Vt, A, Values...> mask) noexcept {
...
template <class To, class A = default_arch, class From>
XSIMD_INLINE void store_as(std::complex<To>* dst, batch<std::complex<From>, A> const& src,
                           aligned_mode) noexcept {
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
template <class To, class A = default_arch, class From, bool i3ec>
XSIMD_INLINE void store_as(xtl::xcomplex<To, To, i3ec>* dst,
                           batch<std::complex<From>, A> const& src, aligned_mode) noexcept {
...
template <class To, class A = default_arch, class From, bool i3ec>
XSIMD_INLINE void store_as(xtl::xcomplex<To, To, i3ec>* dst,
                           batch<std::complex<From>, A> const& src, unaligned_mode) noexcept {
    detail::static_check_supported_config<std::complex<From>, A>();
...
template <class T, class A, class Vt, Vt... Values>
XSIMD_INLINE batch<std::complex<T>, A> swizzle(batch<std::complex<T>, A> const& x,
                                               batch_constant<Vt, A, Values...> mask) noexcept {
    static_assert(sizeof(T) == sizeof(Vt), "consistent mask");
    static_assert(std::is_unsigned<Vt>::value, "mask must hold unsigned indices");
    detail::static_check_supported_config<T, A>();
...
template <class T, class A,
          typename std::enable_if<std::is_floating_point<T>::value, int>::type = 3>
XSIMD_INLINE batch<T, A> bitwise_cast(batch_bool<T, A> const& self) noexcept {
    T z0(0), z1(0);
