 ...
 */
template <size_t N> struct emulated : common {
...
namespace types {
template <size_t N> struct simd_emulated_bool_register {
...
template <typename T, size_t N> struct get_bool_simd_register<T, emulated<N>> {
    using type = simd_emulated_bool_register<N / (8 * sizeof(T))>;
};

template <typename T, size_t N> struct simd_register<T, emulated<N>> {
...
template <typename T, size_t N> struct has_simd_register<T, emulated<N>> : std::is_scalar<T> {};
