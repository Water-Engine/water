...
template <typename T, class A, bool... Values> struct batch_bool_constant {
  ...
  public:
    ...
    static constexpr int mask() noexcept { return mask_helper(0, static_cast<int>(Values)...); }
  ...
  private:
    ...
    static constexpr batch_bool_constant<
        T, A,
        F()(std::tuple_element<Indices, SelfPack>::type::value,
            std::tuple_element<Indices, OtherPack>::type::value)...>
    apply(detail::index_sequence<Indices...>) {
        return {};
    }

    template <class F, bool... OtherValues>
    static constexpr auto apply(batch_bool_constant<T, A, Values...>,
                                batch_bool_constant<T, A, OtherValues...>)
        -> decltype(apply<F, std::tuple<std::integral_constant<bool, Values>...>,
                          std::tuple<std::integral_constant<bool, OtherValues>...>>(
            detail::make_index_sequence<sizeof...(Values)>())) {
        static_assert(sizeof...(Values) == sizeof...(OtherValues), "compatible constant batches");
        return apply<F, std::tuple<std::integral_constant<bool, Values>...>,
                     std::tuple<std::integral_constant<bool, OtherValues>...>>(
            detail::make_index_sequence<sizeof...(Values)>());
    }
...
#define MAKE_BINARY_OP(OP, NAME)                                                                   \
    ...
    constexpr auto operator OP(batch_bool_constant<T, A, OtherValues...> other)                    \
        const->decltype(apply<NAME>(*this, other)) {                                               \
...
#undef MAKE_BINARY_OP
    ...
    constexpr batch_bool_constant<T, A, !Values...> operator!() const { return {}; }
...
template <typename T, class A, T... Values> struct batch_constant {
     ...
     */
    constexpr T get(size_t i) const noexcept { return get(i, std::array<T, size>{Values...}); }
  ...
  private:
    ...
    template <class F, class SelfPack, class OtherPack, size_t... Indices>
    static constexpr batch_constant<T, A,
                                    F()(std::tuple_element<Indices, SelfPack>::type::value,
                                        std::tuple_element<Indices, OtherPack>::type::value)...>
    apply(detail::index_sequence<Indices...>) {
        return {};
    ...
    static constexpr auto apply(batch_constant<T, A, Values...>,
                                batch_constant<T, A, OtherValues...>)
        -> decltype(apply<F, std::tuple<std::integral_constant<T, Values>...>,
                          std::tuple<std::integral_constant<T, OtherValues>...>>(
            detail::make_index_sequence<sizeof...(Values)>())) {
        static_assert(sizeof...(Values) == sizeof...(OtherValues), "compatible constant batches");
        return apply<F, std::tuple<std::integral_constant<T, Values>...>,
                     std::tuple<std::integral_constant<T, OtherValues>...>>(
            detail::make_index_sequence<sizeof...(Values)>());
    }
...
#define MAKE_BINARY_OP(OP, NAME)                                                                   \
    ...
    constexpr auto operator OP(batch_constant<T, A, OtherValues...> other)                         \
        const->decltype(apply<NAME>(*this, other)) {                                               \
        return apply<NAME>(*this, other);                                                          \
    }
...
#undef MAKE_BINARY_OP
    ...
    constexpr batch_constant<T, A, (T)-Values...> operator-() const { return {}; }
 ...
 * generated batch_constant has value `{G::get(0, batch_size), ... , G::get(batch_size - 1,
 * batch_size)}`
 *
...
XSIMD_INLINE constexpr auto make_batch_constant() noexcept
    -> decltype(detail::make_batch_constant<T, G, A>(
