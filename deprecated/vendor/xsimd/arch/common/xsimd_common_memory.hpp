...
// broadcast
namespace detail {
template <class T, class A> struct broadcaster {
    using return_type = batch<T, A>;

    static XSIMD_INLINE return_type run(T v) noexcept { return return_type::broadcast(v); }
};

template <class A> struct broadcaster<bool, A> {
    using return_type = batch_bool<xsimd::as_unsigned_integer_t<bool>, A>;

    static XSIMD_INLINE return_type run(bool b) noexcept { return return_type(b); }
};
} // namespace detail
...
XSIMD_INLINE batch<IT, A> create_compress_swizzle_mask(I bitmask,
                                                       ::xsimd::detail::index_sequence<Is...>) {
...
XSIMD_INLINE batch<T, A> compress(batch<T, A> const& x, batch_bool<T, A> const& mask,
    ...
    auto compress_mask = detail::create_compress_swizzle_mask<IT, A>(
        bitmask, ::xsimd::detail::make_index_sequence<size>());
...
namespace detail {
template <class IT, class A, class I, size_t... Is>
XSIMD_INLINE batch<IT, A> create_expand_swizzle_mask(I bitmask,
                                                     ::xsimd::detail::index_sequence<Is...>) {
    batch<IT, A> swizzle_mask(IT(0));
    IT j = 0;
    (void)std::initializer_list<bool>{((swizzle_mask = insert(swizzle_mask, j, index<Is>())),
                                       (j += ((bitmask >> Is) & 1u)), true)...};
    return swizzle_mask;
}
} // namespace detail

template <typename A, typename T>
XSIMD_INLINE batch<T, A> expand(batch<T, A> const& x, batch_bool<T, A> const& mask,
                                kernel::requires_arch<common>) noexcept {
    constexpr std::size_t size = batch_bool<T, A>::size;
    auto bitmask = mask.mask();
    auto swizzle_mask = detail::create_expand_swizzle_mask<as_unsigned_integer_t<T>, A>(
        bitmask, ::xsimd::detail::make_index_sequence<size>());
...
XSIMD_INLINE batch<T, A> extract_pair(batch<T, A> const& self, batch<T, A> const& other,
                                      std::size_t i, requires_arch<common>) noexcept {
...
template <size_t N, typename T, typename A, typename U, typename V,
          typename std::enable_if<N == 0, int>::type = 0>
inline batch<T, A> gather(U const* src, batch<V, A> const& index, ::xsimd::index<N> I) noexcept {
...
XSIMD_INLINE batch<T, A> gather(batch<T, A> const&, T const* src, batch<V, A> const& index,
                                kernel::requires_arch<common>) noexcept {
    static_assert(batch<T, A>::size == batch<V, A>::size, "Index and destination sizes must match");
...
XSIMD_INLINE batch<T, A> insert(batch<T, A> const& self, T val, index<I>,
    ...
    struct index_mask {
        static constexpr bool get(size_t index, size_t /* size*/) { return index != I; }
    };
    batch<T, A> tmp(val);
...
template <class A, size_t I, class T>
XSIMD_INLINE T get(batch_bool<T, A> const& self, ::xsimd::index<I>,
                   requires_arch<common>) noexcept {
...
template <class A, size_t I, class T>
XSIMD_INLINE auto get(batch<std::complex<T>, A> const& self, ::xsimd::index<I>,
                      requires_arch<common>) noexcept ->
    typename batch<std::complex<T>, A>::value_type {
...
template <class A, class T>
XSIMD_INLINE auto get(batch<std::complex<T>, A> const& self, std::size_t i,
                      requires_arch<common>) noexcept ->
    typename batch<std::complex<T>, A>::value_type {
...
template <class A, class T>
XSIMD_INLINE auto first(batch<std::complex<T>, A> const& self, requires_arch<common>) noexcept ->
    typename batch<std::complex<T>, A>::value_type {
...
template <class A, class T>
XSIMD_INLINE batch_bool<T, A> load_aligned(bool const* mem, batch_bool<T, A> b,
                                           requires_arch<common>) noexcept {
...
XSIMD_INLINE batch<T_out, A> load_aligned(T_in const* mem, convert<T_out>, requires_arch<common>,
                                          with_fast_conversion) noexcept {
...
template <class A, class T_in, class T_out>
XSIMD_INLINE batch<T_out, A> load_aligned(T_in const* mem, convert<T_out>, requires_arch<common>,
                                          with_slow_conversion) noexcept {
    static_assert(!std::is_same<T_in, T_out>::value,
                  "there should be a direct load for this type combination");
    using batch_type_out = batch<T_out, A>;
...
template <class A, class T_in, class T_out>
XSIMD_INLINE batch<T_out, A> load_aligned(T_in const* mem, convert<T_out> cvt,
                                          requires_arch<common>) noexcept {
...
namespace detail {
template <class A, class T_in, class T_out>
XSIMD_INLINE batch<T_out, A> load_unaligned(T_in const* mem, convert<T_out>, requires_arch<common>,
                                            with_fast_conversion) noexcept {
    using batch_type_in = batch<T_in, A>;
    using batch_type_out = batch<T_out, A>;
...
template <class A, class T_in, class T_out>
XSIMD_INLINE batch<T_out, A> load_unaligned(T_in const* mem, convert<T_out> cvt,
...
XSIMD_INLINE batch<T, A> rotate_right(batch<T, A> const& self, requires_arch<common>) noexcept {
    struct rotate_generator {
        static constexpr size_t get(size_t index, size_t size) { return (index - N) % size; }
    };
...
template <size_t N, class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> rotate_right(batch<std::complex<T>, A> const& self,
                                                    requires_arch<common>) noexcept {
...
template <size_t N, class A, class T>
XSIMD_INLINE batch<std::complex<T>, A> rotate_left(batch<std::complex<T>, A> const& self,
                                                   requires_arch<common>) noexcept {
...
template <size_t N, typename T, typename A, typename U, typename V,
          typename std::enable_if<N == 0, int>::type = 0>
XSIMD_INLINE void scatter(batch<T, A> const& src, U* dst, batch<V, A> const& index,
...
XSIMD_INLINE void scatter(batch<T, A> const& src, U* dst, batch<V, A> const& index,
    ...
    kernel::detail::scatter<N - 1, T, A, U, V>(src, dst, index, {});
    dst[index.get(I)] = static_cast<U>(src.get(I));
}
} // namespace detail

template <typename A, typename T, typename V>
XSIMD_INLINE void scatter(batch<T, A> const& src, T* dst, batch<V, A> const& index,
                          kernel::requires_arch<common>) noexcept {
    static_assert(batch<T, A>::size == batch<V, A>::size, "Source and index sizes must match");
    kernel::detail::scatter<batch<V, A>::size - 1, T, A, T, V>(src, dst, index, {});
}

template <typename A, typename T, typename U, typename V>
XSIMD_INLINE detail::sizes_mismatch_t<T, U, void> scatter(batch<T, A> const& src, U* dst,
                                                          batch<V, A> const& index,
                                                          kernel::requires_arch<common>) noexcept {
    static_assert(batch<T, A>::size == batch<V, A>::size, "Source and index sizes must match");
    kernel::detail::scatter<batch<V, A>::size - 1, T, A, U, V>(src, dst, index, {});
}

template <typename A, typename T, typename U, typename V>
XSIMD_INLINE detail::stride_match_t<T, U, void> scatter(batch<T, A> const& src, U* dst,
                                                        batch<V, A> const& index,
                                                        kernel::requires_arch<common>) noexcept {
    static_assert(batch<T, A>::size == batch<V, A>::size, "Source and index sizes must match");
...
// shuffle
namespace detail {
constexpr bool is_swizzle_fst(size_t) { return true; }
...
constexpr bool is_swizzle_snd(size_t) { return true; }
template <typename ITy, typename... ITys>
constexpr bool is_swizzle_snd(size_t bsize, ITy index, ITys... indices) {
    return index >= bsize && is_swizzle_snd(bsize, indices...);
}

constexpr bool is_zip_lo(size_t) { return true; }

template <typename ITy> constexpr bool is_zip_lo(size_t, ITy) { return false; }
...
constexpr bool is_zip_lo(size_t bsize, ITy0 index0, ITy1 index1, ITys... indices) {
    return index0 == (bsize - (sizeof...(indices) + 2)) &&
           index1 == (2 * bsize - (sizeof...(indices) + 2)) && is_zip_lo(bsize, indices...);
}

constexpr bool is_zip_hi(size_t) { return true; }

template <typename ITy> constexpr bool is_zip_hi(size_t, ITy) { return false; }

template <typename ITy0, typename ITy1, typename... ITys>
constexpr bool is_zip_hi(size_t bsize, ITy0 index0, ITy1 index1, ITys... indices) {
    return index0 == (bsize / 2 + bsize - (sizeof...(indices) + 2)) &&
           index1 == (bsize / 2 + 2 * bsize - (sizeof...(indices) + 2)) &&
           is_zip_hi(bsize, indices...);
}

constexpr bool is_select(size_t) { return true; }

template <typename ITy, typename... ITys>
constexpr bool is_select(size_t bsize, ITy index, ITys... indices) {
    return (index < bsize ? index : index - bsize) == (bsize - sizeof...(ITys)) &&
           is_select(bsize, indices...);
}
...
XSIMD_INLINE batch<T, A> shuffle(batch<T, A> const& x, batch<T, A> const& y,
    ...
    XSIMD_IF_CONSTEXPR(detail::is_swizzle_fst(bsize, Indices...)) {
        return swizzle(
            x, batch_constant<ITy, A, ((Indices >= bsize) ? 0 /* never happens */ : Indices)...>());
    }

    XSIMD_IF_CONSTEXPR(detail::is_swizzle_snd(bsize, Indices...)) {
        return swizzle(
            y,
            batch_constant<ITy, A,
                           ((Indices >= bsize) ? (Indices - bsize) : 0 /* never happens */)...>());
    }

    XSIMD_IF_CONSTEXPR(detail::is_zip_lo(bsize, Indices...)) { return zip_lo(x, y); }

    XSIMD_IF_CONSTEXPR(detail::is_zip_hi(bsize, Indices...)) { return zip_hi(x, y); }
...
#define builtin_shuffle __builtin_shufflevector
    ...
    return (typename batch<T, A>::register_type)builtin_shuffle((vty)x.data, (vty)y.data,
                                                                Indices...);
...
//            return __builtin_shuffle(x.data, y.data, integer_vector_type{Indices...});
    ...
    // pretty well.
    batch<T, A> x_lane =
        swizzle(x, batch_constant<ITy, A, ((Indices >= bsize) ? (Indices - bsize) : Indices)...>());
    batch<T, A> y_lane =
        swizzle(y, batch_constant<ITy, A, ((Indices >= bsize) ? (Indices - bsize) : Indices)...>());
...
// store_aligned
template <class A, class T_in, class T_out>
XSIMD_INLINE void store_aligned(T_out* mem, batch<T_in, A> const& self,
                                requires_arch<common>) noexcept {
    static_assert(!std::is_same<T_in, T_out>::value,
                  "there should be a direct store for this type combination");
...
// store_unaligned
template <class A, class T_in, class T_out>
XSIMD_INLINE void store_unaligned(T_out* mem, batch<T_in, A> const& self,
                                  requires_arch<common>) noexcept {
    static_assert(!std::is_same<T_in, T_out>::value,
                  "there should be a direct store for this type combination");
...
XSIMD_INLINE batch<std::complex<T>, A> swizzle(batch<std::complex<T>, A> const& self,
                                               batch_constant<ITy, A, Vs...> mask,
                                               requires_arch<common>) noexcept {
    return {swizzle(self.real(), mask), swizzle(self.imag(), mask)};
}

template <class A, class T, class ITy>
XSIMD_INLINE batch<T, A> swizzle(batch<T, A> const& self, batch<ITy, A> mask,
                                 requires_arch<common>) noexcept {
...
template <class A, class T, class ITy>
XSIMD_INLINE batch<std::complex<T>, A> swizzle(batch<std::complex<T>, A> const& self,
                                               batch<ITy, A> mask, requires_arch<common>) noexcept {
    return {swizzle(self.real(), mask), swizzle(self.imag(), mask)};
}
...
load_complex(batch<T, A> const& /*hi*/, batch<T, A> const& /*lo*/, requires_arch<common>) noexcept {
    static_assert(std::is_same<T, void>::value,
                  "load_complex not implemented for the required architecture");
}

template <class A, class T>
XSIMD_INLINE batch<T, A> complex_high(batch<std::complex<T>, A> const& /*src*/,
                                      requires_arch<common>) noexcept {
    static_assert(std::is_same<T, void>::value,
                  "complex_high not implemented for the required architecture");
}

template <class A, class T>
XSIMD_INLINE batch<T, A> complex_low(batch<std::complex<T>, A> const& /*src*/,
                                     requires_arch<common>) noexcept {
    static_assert(std::is_same<T, void>::value,
                  "complex_low not implemented for the required architecture");
}
} // namespace detail

template <class A, class T_out, class T_in>
XSIMD_INLINE batch<std::complex<T_out>, A> load_complex_aligned(std::complex<T_in> const* mem,
                                                                convert<std::complex<T_out>>,
                                                                requires_arch<common>) noexcept {
...
template <class A, class T_out, class T_in>
XSIMD_INLINE batch<std::complex<T_out>, A> load_complex_unaligned(std::complex<T_in> const* mem,
                                                                  convert<std::complex<T_out>>,
                                                                  requires_arch<common>) noexcept {
    using real_batch = batch<T_out, A>;
    T_in const* buffer = reinterpret_cast<T_in const*>(mem);
...
template <class A, class T_out, class T_in>
XSIMD_INLINE void store_complex_aligned(std::complex<T_out>* dst,
                                        batch<std::complex<T_in>, A> const& src,
                                        requires_arch<common>) noexcept {
...
template <class A, class T_out, class T_in>
XSIMD_INLINE void store_complex_unaligned(std::complex<T_out>* dst,
                                          batch<std::complex<T_in>, A> const& src,
                                          requires_arch<common>) noexcept {
    using real_batch = batch<T_in, A>;
    real_batch hi = detail::complex_high(src, A{});
...
XSIMD_INLINE void transpose(batch<int16_t, A>* matrix_begin, batch<int16_t, A>* matrix_end,
    ...
    matrix_begin[0] = bit_cast<batch<int16_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(l4), bit_cast<batch<int64_t, A>>(l5)));
    matrix_begin[1] = bit_cast<batch<int16_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(l4), bit_cast<batch<int64_t, A>>(l5)));
    matrix_begin[2] = bit_cast<batch<int16_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(l6), bit_cast<batch<int64_t, A>>(l7)));
    matrix_begin[3] = bit_cast<batch<int16_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(l6), bit_cast<batch<int64_t, A>>(l7)));

    matrix_begin[4] = bit_cast<batch<int16_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(h4), bit_cast<batch<int64_t, A>>(h5)));
    matrix_begin[5] = bit_cast<batch<int16_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(h4), bit_cast<batch<int64_t, A>>(h5)));
    matrix_begin[6] = bit_cast<batch<int16_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(h6), bit_cast<batch<int64_t, A>>(h7)));
    matrix_begin[7] = bit_cast<batch<int16_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(h6), bit_cast<batch<int64_t, A>>(h7)));
}

template <class A>
XSIMD_INLINE void transpose(batch<uint16_t, A>* matrix_begin, batch<uint16_t, A>* matrix_end,
                            requires_arch<common>) noexcept {
    transpose(reinterpret_cast<batch<int16_t, A>*>(matrix_begin),
              reinterpret_cast<batch<int16_t, A>*>(matrix_end), A{});
}
...
XSIMD_INLINE void transpose(batch<int8_t, A>* matrix_begin, batch<int8_t, A>* matrix_end,
    ...
    matrix_begin[0] = bit_cast<batch<int8_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(m0), bit_cast<batch<int64_t, A>>(m1)));
    matrix_begin[1] = bit_cast<batch<int8_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(m0), bit_cast<batch<int64_t, A>>(m1)));
    matrix_begin[2] = bit_cast<batch<int8_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(m2), bit_cast<batch<int64_t, A>>(m3)));
    matrix_begin[3] = bit_cast<batch<int8_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(m2), bit_cast<batch<int64_t, A>>(m3)));
    ...
    matrix_begin[4] = bit_cast<batch<int8_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(m4), bit_cast<batch<int64_t, A>>(m5)));
    matrix_begin[5] = bit_cast<batch<int8_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(m4), bit_cast<batch<int64_t, A>>(m5)));
    matrix_begin[6] = bit_cast<batch<int8_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(m6), bit_cast<batch<int64_t, A>>(m7)));
    matrix_begin[7] = bit_cast<batch<int8_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(m6), bit_cast<batch<int64_t, A>>(m7)));
    ...
    matrix_begin[8] = bit_cast<batch<int8_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(M0), bit_cast<batch<int64_t, A>>(M1)));
    matrix_begin[9] = bit_cast<batch<int8_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(M0), bit_cast<batch<int64_t, A>>(M1)));
    matrix_begin[10] = bit_cast<batch<int8_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(M2), bit_cast<batch<int64_t, A>>(M3)));
    matrix_begin[11] = bit_cast<batch<int8_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(M2), bit_cast<batch<int64_t, A>>(M3)));
    ...
    matrix_begin[12] = bit_cast<batch<int8_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(M4), bit_cast<batch<int64_t, A>>(M5)));
    matrix_begin[13] = bit_cast<batch<int8_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(M4), bit_cast<batch<int64_t, A>>(M5)));
    matrix_begin[14] = bit_cast<batch<int8_t, A>>(
        zip_lo(bit_cast<batch<int64_t, A>>(M6), bit_cast<batch<int64_t, A>>(M7)));
    matrix_begin[15] = bit_cast<batch<int8_t, A>>(
        zip_hi(bit_cast<batch<int64_t, A>>(M6), bit_cast<batch<int64_t, A>>(M7)));
}
...
XSIMD_INLINE void transpose(batch<uint8_t, A>* matrix_begin, batch<uint8_t, A>* matrix_end,
                            requires_arch<common>) noexcept {
    transpose(reinterpret_cast<batch<int8_t, A>*>(matrix_begin),
              reinterpret_cast<batch<int8_t, A>*>(matrix_end), A{});
}
