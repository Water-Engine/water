...
XSIMD_INLINE signed char abs(signed char v) { return v < 0 ? -v : v; }

namespace detail {
// Use templated type here to prevent automatic instantiation that may
// ends up in a warning
template <typename char_type> XSIMD_INLINE char abs(char_type v, std::true_type) { return v; }
template <typename char_type> XSIMD_INLINE char abs(char_type v, std::false_type) {
    return v < 0 ? -v : v;
}
} // namespace detail

XSIMD_INLINE char abs(char v) { return detail::abs(v, std::is_unsigned<char>::type{}); }

XSIMD_INLINE short abs(short v) { return v < 0 ? -v : v; }
XSIMD_INLINE unsigned char abs(unsigned char v) { return v; }
XSIMD_INLINE unsigned short abs(unsigned short v) { return v; }
XSIMD_INLINE unsigned int abs(unsigned int v) { return v; }
XSIMD_INLINE unsigned long abs(unsigned long v) { return v; }
XSIMD_INLINE unsigned long long abs(unsigned long long v) { return v; }
...
template <class T>
XSIMD_INLINE typename std::enable_if<std::is_integral<T>::value, bool>::type isinf(T var) noexcept {
...
template <class T>
XSIMD_INLINE typename std::enable_if<std::is_integral<T>::value, bool>::type isnan(T var) noexcept {
...
template <class T> XSIMD_INLINE T incr(T const& x) noexcept { return x + T(1); }

template <class T> XSIMD_INLINE T incr_if(T const& x, bool mask) noexcept {
    return x + T(mask ? 1 : 0);
}

XSIMD_INLINE bool all(bool mask) { return mask; }

XSIMD_INLINE bool any(bool mask) { return mask; }

XSIMD_INLINE bool none(bool mask) { return !mask; }
...
template <class T_out, class T_in> XSIMD_INLINE T_out bitwise_cast(T_in x) noexcept {
...
XSIMD_INLINE
    typename std::enable_if<std::is_integral<T0>::value && std::is_integral<T1>::value, T0>::type
...
XSIMD_INLINE bool bitwise_not(bool x) noexcept { return !x; }
...
template <class T>
XSIMD_INLINE typename std::enable_if<std::is_scalar<T>::value, T>::type
bitwise_andnot(T x, T y) noexcept {
    return bitwise_and(x, bitwise_not(y));
}

template <class T>
XSIMD_INLINE typename std::enable_if<std::is_integral<T>::value, T>::type bitwise_or(T x,
                                                                                     T y) noexcept {
    return x | y;
}
...
template <class T> XSIMD_INLINE T neg(T const& x) noexcept { return -x; }

template <class T> XSIMD_INLINE auto pos(T const& x) noexcept -> decltype(+x) { return +x; }

XSIMD_INLINE float reciprocal(float const& x) noexcept { return 1.f / x; }

XSIMD_INLINE double reciprocal(double const& x) noexcept { return 1. / x; }
...
template <class T> XSIMD_INLINE bool isnan(std::complex<T> var) noexcept {
    return std::isnan(std::real(var)) || std::isnan(std::imag(var));
}

template <class T> XSIMD_INLINE bool isinf(std::complex<T> var) noexcept {
    return std::isinf(std::real(var)) || std::isinf(std::imag(var));
}

template <class T> XSIMD_INLINE bool isfinite(std::complex<T> var) noexcept {
...
XSIMD_INLINE float exp10(const float& x) noexcept { return __exp10f(x); }
XSIMD_INLINE double exp10(const double& x) noexcept { return __exp10(x); }
#elif defined(__GLIBC__)
XSIMD_INLINE float exp10(const float& x) noexcept { return ::exp10f(x); }
XSIMD_INLINE double exp10(const double& x) noexcept { return ::exp10(x); }
#elif !defined(__clang__) && defined(__GNUC__) && (__GNUC__ >= 5)
XSIMD_INLINE float exp10(const float& x) noexcept { return __builtin_exp10f(x); }
XSIMD_INLINE double exp10(const double& x) noexcept { return __builtin_exp10(x); }
#elif defined(_WIN32)
template <class T, class = typename std::enable_if<std::is_scalar<T>::value>::type>
...
namespace detail {
template <class C> XSIMD_INLINE C expm1_complex_scalar_impl(const C& val) noexcept {
...
template <class T> XSIMD_INLINE std::complex<T> expm1(const std::complex<T>& val) noexcept {
...
namespace detail {
template <class C> XSIMD_INLINE C log1p_complex_scalar_impl(const C& val) noexcept {
    using T = typename C::value_type;
    C u = C(1.) + val;
...
template <class T> XSIMD_INLINE std::complex<T> log1p(const std::complex<T>& val) noexcept {
    return detail::log1p_complex_scalar_impl(val);
}

template <class T> XSIMD_INLINE std::complex<T> log2(const std::complex<T>& val) noexcept {
...
template <class T> using value_type_or_type = typename value_type_or_type_helper<T>::type;
...
template <class T0, class T1>
XSIMD_INLINE auto pow(const T0& t0, const T1& t1) noexcept ->
    typename std::enable_if<std::is_scalar<T0>::value && std::is_floating_point<T1>::value,
                            decltype(std::pow(t0, t1))>::type {
...
template <class T0, class T1>
XSIMD_INLINE auto pow(const T0& t0, const std::complex<T1>& t1) noexcept ->
...
XSIMD_INLINE float bitofsign(float const& x) noexcept { return float(std::signbit(x)); }

XSIMD_INLINE double bitofsign(double const& x) noexcept { return double(std::signbit(x)); }

XSIMD_INLINE long double bitofsign(long double const& x) noexcept {
    return static_cast<long double>(std::signbit(x));
}

template <class T> XSIMD_INLINE auto signbit(T const& v) noexcept -> decltype(bitofsign(v)) {
    return bitofsign(v);
}

XSIMD_INLINE double sign(bool const& v) noexcept { return v; }
...
namespace detail {
template <class C> XSIMD_INLINE C sign_complex_scalar_impl(const C& v) noexcept {
...
template <class T> XSIMD_INLINE std::complex<T> sign(const std::complex<T>& v) noexcept {
...
XSIMD_INLINE double signnz(bool const&) noexcept { return 1; }
...
template <class T> XSIMD_INLINE T decr(T const& x) noexcept { return x - T(1); }

template <class T> XSIMD_INLINE T decr_if(T const& x, bool mask) noexcept {
    return x - T(mask ? 1 : 0);
}
...
template <class T0, class T1>
XSIMD_INLINE auto min(T0 const& self, T1 const& other) noexcept -> typename std::enable_if<
    std::is_scalar<T0>::value && std::is_scalar<T1>::value,
...
min(std::complex<T0> const& self, std::complex<T1> const& other) noexcept {
    return (self.real() < other.real())
               ? (self)
               : (self.real() == other.real() ? (self.imag() < other.imag() ? self : other)
                                              : other);
}

template <class T0, class T1>
XSIMD_INLINE auto max(T0 const& self, T1 const& other) noexcept -> typename std::enable_if<
    std::is_scalar<T0>::value && std::is_scalar<T1>::value,
    typename std::decay<decltype(self > other ? other : self)>::type>::type {
    return self < other ? other : self;
}
...
template <class T>
XSIMD_INLINE typename std::enable_if<std::is_floating_point<T>::value, T>::type
fma(const T& a, const T& b, const T& c) noexcept {
    return std::fma(a, b, c);
}

template <class T>
XSIMD_INLINE typename std::enable_if<std::is_scalar<T>::value, T>::type fms(const T& a, const T& b,
                                                                            const T& c) noexcept {
    return a * b - c;
}
...
template <class T>
XSIMD_INLINE std::complex<T> fma(const std::complex<T>& a, const std::complex<T>& b,
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
template <class T, bool i3ec>
XSIMD_INLINE xtl::xcomplex<T, T, i3ec> fma(const xtl::xcomplex<T, T, i3ec>& a,
                                           const xtl::xcomplex<T, T, i3ec>& b,
                                           const xtl::xcomplex<T, T, i3ec>& c) noexcept {
    return detail::fma_complex_scalar_impl(a, b, c);
}
#endif
...
template <class T>
XSIMD_INLINE std::complex<T> fms(const std::complex<T>& a, const std::complex<T>& b,
                                 const std::complex<T>& c) noexcept {
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
template <class T, bool i3ec>
XSIMD_INLINE xtl::xcomplex<T, T, i3ec> fms(const xtl::xcomplex<T, T, i3ec>& a,
                                           const xtl::xcomplex<T, T, i3ec>& b,
                                           const xtl::xcomplex<T, T, i3ec>& c) noexcept {
    return detail::fms_complex_scalar_impl(a, b, c);
}
...
template <class T>
XSIMD_INLINE std::complex<T> fnma(const std::complex<T>& a, const std::complex<T>& b,
                                  const std::complex<T>& c) noexcept {
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
template <class T, bool i3ec>
XSIMD_INLINE xtl::xcomplex<T, T, i3ec> fnma(const xtl::xcomplex<T, T, i3ec>& a,
                                            const xtl::xcomplex<T, T, i3ec>& b,
                                            const xtl::xcomplex<T, T, i3ec>& c) noexcept {
    return detail::fnma_complex_scalar_impl(a, b, c);
}
...
template <class T>
XSIMD_INLINE std::complex<T> fnms(const std::complex<T>& a, const std::complex<T>& b,
                                  const std::complex<T>& c) noexcept {
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
template <class T, bool i3ec>
XSIMD_INLINE xtl::xcomplex<T, T, i3ec> fnms(const xtl::xcomplex<T, T, i3ec>& a,
                                            const xtl::xcomplex<T, T, i3ec>& b,
                                            const xtl::xcomplex<T, T, i3ec>& c) noexcept {
    return detail::fnms_complex_scalar_impl(a, b, c);
}
...
#define XSIMD_HASSINCOS_TRAIT(func)                                                                \
    template <class S> struct has##func {                                                          \
        template <class T>                                                                         \
        static XSIMD_INLINE auto get(T* ptr)                                                       \
            -> decltype(func(std::declval<T>(), std::declval<T*>(), std::declval<T*>()),           \
                        std::true_type{});                                                         \
...
struct common_sincosf {
    ...
    template <class T>
    XSIMD_INLINE
        typename std::enable_if<!XSIMD_HASSINCOS(sincosf, T) && XSIMD_HASSINCOS(__sincosf, T),
                                void>::type
        operator()(float val, T& s, T& c) {
        __sincosf(val, &s, &c);
    }
...
struct common_sincos {
    ...
    template <class T>
    XSIMD_INLINE
        typename std::enable_if<!XSIMD_HASSINCOS(sincos, T) && XSIMD_HASSINCOS(__sincos, T),
                                void>::type
        operator()(double val, T& s, T& c) {
        __sincos(val, &s, &c);
    }
...
#ifdef XSIMD_ENABLE_XTL_COMPLEX
template <class T>
XSIMD_INLINE std::pair<xtl::xcomplex<T>, xtl::xcomplex<T>>
sincos(const xtl::xcomplex<T>& val) noexcept {
...
template <class T> XSIMD_INLINE T select(bool cond, T const& true_br, T const& false_br) noexcept {
