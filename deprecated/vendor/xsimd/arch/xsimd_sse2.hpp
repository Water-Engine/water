...
constexpr uint32_t shuffle(uint32_t x, uint32_t y) { return (y << 1) | x; }

constexpr uint32_t mod_shuffle(uint32_t w, uint32_t x, uint32_t y, uint32_t z) {
    return shuffle(w % 4, x % 4, y % 4, z % 4);
}

constexpr uint32_t mod_shuffle(uint32_t w, uint32_t x) { return shuffle(w % 2, x % 2); }
} // namespace detail
...
XSIMD_INLINE batch<T, A> shuffle(batch<T, A> const& x, batch<T, A> const& y,
                                 batch_constant<ITy, A, Indices...>,
                                 requires_arch<common>) noexcept;
template <class A, class T>
XSIMD_INLINE batch<T, A> avg(batch<T, A> const&, batch<T, A> const&,
                             requires_arch<common>) noexcept;
template <class A, class T>
XSIMD_INLINE batch<T, A> avgr(batch<T, A> const&, batch<T, A> const&,
                              requires_arch<common>) noexcept;
...
XSIMD_INLINE batch<T, A> add(batch<T, A> const& self, batch<T, A> const& other,
    ...
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_add_epi8(self, other); }
...
XSIMD_INLINE batch<T, A> avgr(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<sse2>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_avg_epu8(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
// avg
template <class A, class T, class = typename std::enable_if<std::is_unsigned<T>::value, void>::type>
XSIMD_INLINE batch<T, A> avg(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<sse2>) noexcept {
...
template <class A>
batch<double, A> XSIMD_INLINE bitwise_and(batch<double, A> const& self,
    ...
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
            ...
            return _mm_or_si128(
                ...
                _mm_slli_epi64(_mm_srai_epi32(_mm_shuffle_epi32(self, _MM_SHUFFLE(3, 3, 1, 1)), 32),
                               64 - other));
        }
        else {
            assert(false && "unsupported arch/op combination");
...
// broadcast
template <class A> batch<float, A> XSIMD_INLINE broadcast(float val, requires_arch<sse2>) noexcept {
    return _mm_set1_ps(val);
}
template <class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch<T, A> broadcast(T val, requires_arch<sse2>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_set1_epi8(val); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
template <class A>
XSIMD_INLINE batch<double, A> fast_cast(batch<uint64_t, A> const& x, batch<double, A> const&,
                                        requires_arch<sse2>) noexcept {
    // from
    ...
    __m128i xL = _mm_or_si128(
        _mm_and_si128(mask, x),
        _mm_andnot_si128(mask, _mm_castpd_si128(_mm_set1_pd(0x0010000000000000)))); //  2^52
    __m128d f =
        _mm_sub_pd(_mm_castsi128_pd(xH), _mm_set1_pd(19342813118337666422669312.)); //  2^84 + 2^52
    return _mm_add_pd(f, _mm_castsi128_pd(xL));
}

template <class A>
XSIMD_INLINE batch<double, A> fast_cast(batch<int64_t, A> const& x, batch<double, A> const&,
                                        requires_arch<sse2>) noexcept {
    // from
    // https://stackoverflow.com/questions/41144668/how-to-efficiently-perform-double-int64-conversions-with-sse-avx
    // adapted to sse2
    ...
    xH = _mm_and_si128(
        xH, _mm_setr_epi16(0x0000, 0x0000, 0xFFFF, 0xFFFF, 0x0000, 0x0000, 0xFFFF, 0xFFFF));
    ...
    __m128i xL = _mm_or_si128(
        ...
        _mm_andnot_si128(mask, _mm_castpd_si128(_mm_set1_pd(0x0010000000000000)))); //  2^52
    __m128d f =
        _mm_sub_pd(_mm_castsi128_pd(xH), _mm_set1_pd(442726361368656609280.)); //  3*2^67 + 2^52
    return _mm_add_pd(f, _mm_castsi128_pd(xL));
}
...
XSIMD_INLINE batch_bool<T, A> eq(batch<T, A> const& self, batch<T, A> const& other,
                                 requires_arch<sse2>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_cmpeq_epi8(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE T first(batch<T, A> const& self, requires_arch<sse2>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return static_cast<T>(_mm_cvtsi128_si32(self) & 0xFF); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch_bool<T, A> from_mask(batch_bool<T, A> const&, uint64_t mask,
    ...
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
        ...
        return _mm_setr_epi32(lut32[mask & 0xF], lut32[(mask >> 4) & 0xF], lut32[(mask >> 8) & 0xF],
                              lut32[mask >> 12]);
    }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch_bool<T, A> gt(batch<T, A> const& self, batch<T, A> const& other,
                                 requires_arch<sse2>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_cmpgt_epi8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<double, A> haddp(batch<double, A> const* row, requires_arch<sse2>) noexcept {
    return _mm_add_pd(_mm_unpacklo_pd(row[0], row[1]), _mm_unpackhi_pd(row[0], row[1]));
}
...
XSIMD_INLINE batch<T, A> insert(batch<T, A> const& self, T val, index<I> pos,
                                requires_arch<sse2>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 2) { return _mm_insert_epi16(self, val, I); }
    else {
...
load_complex(batch<float, A> const& hi, batch<float, A> const& lo, requires_arch<sse2>) noexcept {
    return {_mm_shuffle_ps(hi, lo, _MM_SHUFFLE(2, 0, 2, 0)),
            _mm_shuffle_ps(hi, lo, _MM_SHUFFLE(3, 1, 3, 1))};
}
template <class A>
XSIMD_INLINE batch<std::complex<double>, A>
load_complex(batch<double, A> const& hi, batch<double, A> const& lo, requires_arch<sse2>) noexcept {
...
XSIMD_INLINE batch_bool<T, A> lt(batch<T, A> const& self, batch<T, A> const& other,
                                 requires_arch<sse2>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_cmplt_epi8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) {
            ...
            auto xself =
                _mm_xor_si128(self, _mm_set1_epi64x(std::numeric_limits<int64_t>::lowest()));
            auto xother =
                _mm_xor_si128(other, _mm_set1_epi64x(std::numeric_limits<int64_t>::lowest()));
...
XSIMD_INLINE uint64_t mask(batch_bool<T, A> const& self, requires_arch<sse2>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_movemask_epi8(self); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
// mul
template <class A>
XSIMD_INLINE batch<int16_t, A> mul(batch<int16_t, A> const& self, batch<int16_t, A> const& other,
                                   requires_arch<sse2>) noexcept {
...
XSIMD_INLINE batch<double, A> neg(batch<double, A> const& self, requires_arch<sse2>) noexcept {
    return _mm_xor_pd(self, _mm_castsi128_pd(_mm_setr_epi32(0, 0x80000000, 0, 0x80000000)));
}
...
// reciprocal
template <class A>
XSIMD_INLINE batch<float, A> reciprocal(batch<float, A> const& self, kernel::requires_arch<sse2>) {
...
XSIMD_INLINE batch<T, A> select(batch_bool<T, A> const& cond, batch<T, A> const& true_br,
                                batch<T, A> const& false_br, requires_arch<sse2>) noexcept {
...
select(batch_bool<double, A> const& cond, batch<double, A> const& true_br,
       batch<double, A> const& false_br, requires_arch<sse2>) noexcept {
...
XSIMD_INLINE batch<double, A> shuffle(batch<double, A> const& x, batch<double, A> const& y,
                                      batch_constant<ITy, A, I0, I1> mask,
                                      requires_arch<sse2>) noexcept {
...
XSIMD_INLINE batch<T, A> sadd(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<sse2>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_adds_epi8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_adds_epu8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<float, A> set(batch<float, A> const&, requires_arch<sse2>,
...
template <class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch<T, A> set(batch<T, A> const&, requires_arch<sse2>, T v0, T v1, T v2,
                             T v3) noexcept {
    return _mm_setr_epi32(v0, v1, v2, v3);
}
template <class A, class T, class = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch<T, A> set(batch<T, A> const&, requires_arch<sse2>, T v0, T v1, T v2, T v3, T v4,
...
template <class A, class... Values>
XSIMD_INLINE batch<double, A> set(batch<double, A> const&, requires_arch<sse2>,
                                  Values... values) noexcept {
...
template <class A, class T, class... Values,
          class = typename std::enable_if<std::is_integral<T>::value, void>::type>
XSIMD_INLINE batch_bool<T, A> set(batch_bool<T, A> const&, requires_arch<sse2>,
                                  Values... values) noexcept {
    return set(batch<T, A>(), A{}, static_cast<T>(values ? -1LL : 0LL)...).data;
}

template <class A, class... Values>
XSIMD_INLINE batch_bool<float, A> set(batch_bool<float, A> const&, requires_arch<sse2>,
                                      Values... values) noexcept {
...
template <class A, class... Values>
XSIMD_INLINE batch_bool<double, A> set(batch_bool<double, A> const&, requires_arch<sse2>,
                                       Values... values) noexcept {
...
XSIMD_INLINE batch<T, A> ssub(batch<T, A> const& self, batch<T, A> const& other,
                              requires_arch<sse2>) noexcept {
    if (std::is_signed<T>::value) {
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_subs_epi8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
        ...
        XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_subs_epu8(self, other); }
        else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> sub(batch<T, A> const& self, batch<T, A> const& other,
                             requires_arch<sse2>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_sub_epi8(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<float, A> swizzle(batch<float, A> const& self,
                                     batch_constant<uint32_t, A, V0, V1, V2, V3>,
                                     requires_arch<sse2>) noexcept {
...
template <class A, uint16_t V0, uint16_t V1, uint16_t V2, uint16_t V3, uint16_t V4, uint16_t V5,
          uint16_t V6, uint16_t V7>
XSIMD_INLINE batch<uint16_t, A>
swizzle(batch<uint16_t, A> const& self,
        batch_constant<uint16_t, A, V0, V1, V2, V3, V4, V5, V6, V7> mask,
        requires_arch<sse2>) noexcept {
    ...
    XSIMD_IF_CONSTEXPR((V0 < 4) && (V1 < 4) && (V2 < 4) && (V3 < 4) && (V4 < 4) && (V5 < 4) &&
                       (V6 < 4) && (V7 < 4)) {
    ...
    batch_bool_constant<uint16_t, A, (V0 < 4), (V1 < 4), (V2 < 4), (V3 < 4), (V4 < 4), (V5 < 4),
                        (V6 < 4), (V7 < 4)>
        blend_mask;
...
swizzle(batch<int16_t, A> const& self,
        batch_constant<uint16_t, A, V0, V1, V2, V3, V4, V5, V6, V7> mask,
        requires_arch<sse2>) noexcept {
...
XSIMD_INLINE void transpose(batch<uint32_t, A>* matrix_begin, batch<uint32_t, A>* matrix_end,
                            requires_arch<sse2>) noexcept {
    transpose(reinterpret_cast<batch<float, A>*>(matrix_begin),
...
XSIMD_INLINE void transpose(batch<uint64_t, A>* matrix_begin, batch<uint64_t, A>* matrix_end,
                            requires_arch<sse2>) noexcept {
    transpose(reinterpret_cast<batch<double, A>*>(matrix_begin),
...
XSIMD_INLINE batch<T, A> zip_hi(batch<T, A> const& self, batch<T, A> const& other,
                                requires_arch<sse2>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_unpackhi_epi8(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
...
XSIMD_INLINE batch<T, A> zip_lo(batch<T, A> const& self, batch<T, A> const& other,
                                requires_arch<sse2>) noexcept {
    XSIMD_IF_CONSTEXPR(sizeof(T) == 1) { return _mm_unpacklo_epi8(self, other); }
    else XSIMD_IF_CONSTEXPR(sizeof(T) == 2) {
